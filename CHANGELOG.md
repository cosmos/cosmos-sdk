...
* (codec) `*codec.Codec` is now a wrapper around Amino which provides proper backwards compatibility with protobuf `Any`. ALL legacy code
should use `*codec.Codec` instead of `*amino.Codec` directly
