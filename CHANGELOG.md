...
* (x/auth) [#XXXXX](https://github.com/cosmos/cosmos-sdk/pull/XXXXX) Keep an unordered transaction's nonce until the block time is strictly past its `timeout_timestamp`. `RemoveExpiredUnorderedNonces` deleted nonces whose timeout equaled the block time, while the ante handler still accepts those transactions in that block, so a signed unordered transaction could be replayed once in the block whose time equals its timeout. Transaction validity is unchanged; only the block in which the nonce is pruned moved.
...
* (deps) [#25820](https://github.com/cosmos/cosmos-sdk/pull/25820) Bump github.com/cometbft/cometbft from 0.30.20 to 0.38.21
