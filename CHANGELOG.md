...
* (baseapp) [#26572](https://github.com/cosmos/cosmos-sdk/pull/26572) Make `BaseApp.Close` idempotent by clearing the `db` and `snapshotManager` handles after closing them. `server.startInProcess` registers two deferred cleanup paths that both call `app.Close()` on shutdown, so the second call re-closed the already-closed application DB and panicked (e.g. PebbleDB's `pebble: closed`), crashing every otherwise-graceful shutdown with a non-zero exit.
