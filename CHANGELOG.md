...
* (baseapp) [#26056](https://github.com/cosmos/cosmos-sdk/pull/26056) Remove `BaseApp.NewUncachedContext()`. Tests that used `BaseApp.NewUncachedContext(false, header)` should now use `BaseApp.NewNextBlockContext(header)`. Tests that used `BaseApp.NewUncachedContext(true, header)` (with `isCheckTx` true) should adapt to `BaseApp.NewContext(true)`, but mostly that usage in tests was wrong. `BaseApp.SimWriteState` was deprecated and will be removed in a future version.
