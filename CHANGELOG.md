...
* (x/authz) [#26999](https://github.com/cosmos/cosmos-sdk/pull/26999) Cap the number of expired grants pruned per `BeginBlocker` call to 200, matching `x/feegrant`'s existing pattern, so a block where many grants expire at once can't cause unbounded work.
