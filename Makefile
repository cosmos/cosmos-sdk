...
localnet-start: build-linux localnet-stop
	...
	if ! [ -f build/localnet/node0/simd/config/genesis.json ]; then $(DOCKER) run --rm \
		--user $(shell id -u):$(shell id -g) \
		-v $(BUILDDIR):/simd:Z \
		-v /etc/group:/etc/group:ro \
		-v /etc/passwd:/etc/passwd:ro \
		-v /etc/shadow:/etc/shadow:ro \
		cosmossdk/simd-env testnet init-files --chain-id localnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
