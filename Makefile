...
proto-gen:
	...
	if docker ps -a --format '{{.Names}}' | grep -Eq "^${protoContainer}"; then docker start $(protoContainer); else docker run --name $(protoContainer) -v $(CURDIR):/workspace --workdir /workspace tendermintdev/sdk-proto-gen:v0.1 sh ./scripts/protocgen.sh; fi
