...
Several gas-related parameters have been added to the client config. Using SimApp as an example, to utilize this feature, add the following lines to `~/.simapp/config/client.toml`:
...
These parameters are optional. If not specified, default values (same as with previous Cosmos SDK versions) will be used.
