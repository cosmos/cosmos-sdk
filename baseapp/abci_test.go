...
func anyMessage(t *testing.T, cdc codec.Codec, msg *baseapptestutil.MsgSend) *types.Any {
	t.Helper()
...
func TestABCI_Query_SimulateNestedMessagesTx(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			nestedMessages := make([]*types.Any, 1)
			b, err := suite.cdc.Marshal(tt.message)
			require.NoError(t, err)
			nestedMessages[0] = &types.Any{
...
func TestABCI_Query_SimulateNestedMessagesGas(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := tt.suite.baseApp.InitChain(&abci.RequestInitChain{
				ConsensusParams: &cmtproto.ConsensusParams{},
			})
			require.NoError(t, err)
