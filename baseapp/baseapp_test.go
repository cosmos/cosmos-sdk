...
func setupBaseAppWithSnapshots(t *testing.T, blocks uint, blockTxs int, options ...func(*BaseApp)) (*BaseApp, func()) {
	codec := codec.NewLegacyAmino()
	registerTestCodec(codec)
	routerOpt := func(bapp *BaseApp) { bapp.SetTxHandler(nil) }
...
func TestLoadVersion(t *testing.T) {
	logger := defaultLogger()
	pruningOpt := SetPruning(storetypes.PruneNothing)
	db := dbm.NewMemDB()
	name := t.Name()
...
func TestSetLoader(t *testing.T) {
	...
	for name, tc := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			opts := []func(*BaseApp){SetPruning(storetypes.PruneNothing)}
			if tc.setLoader != nil {
...
func TestVersionSetterGetter(t *testing.T) {
	logger := defaultLogger()
	pruningOpt := SetPruning(storetypes.PruneDefault)
	db := dbm.NewMemDB()
	name := t.Name()
	app := NewBaseApp(name, logger, db, nil, pruningOpt)
