	...
	testCases := []struct {
	...
	}{
		{
			...
			func(t *testing.T, bap *baseapp.BaseApp) {
				t.Helper()
				bap.SetBlockSTMTxRunner(txnrunner.NewSTMRunner(nil, nil, 0, true, nil), t.TempDir())
		...
		},
		{
			...
			func(t *testing.T, bap *baseapp.BaseApp) {
				...
				require.Panics(t, func() { bap.SetBlockSTMTxRunner(txnrunner.NewSTMRunner(nil, nil, 0, true, nil), t.TempDir()) })
			},
		},
		{
			...
			func(t *testing.T, bap *baseapp.BaseApp) {
				t.Helper()
				require.NotPanics(t, func() { bap.SetBlockSTMTxRunner(txnrunner.NewSTMRunner(nil, nil, 0, true, nil), t.TempDir()) })
			},
		},
	}
