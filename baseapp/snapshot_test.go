...
func TestABCI_OfferSnapshot_Errors(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, &abci.OfferSnapshotResponse{Result: abci.OfferSnapshotResponse_ACCEPT}, resp)
	...
	require.NoError(t, err)
	require.Equal(t, &abci.OfferSnapshotResponse{Result: abci.OfferSnapshotResponse_ABORT}, resp)
}
...
func TestABCI_ApplySnapshotChunk(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, &abci.OfferSnapshotResponse{Result: abci.OfferSnapshotResponse_ACCEPT}, respOffer)
	...
	require.Equal(t, &abci.ApplySnapshotChunkResponse{
		Result:        abci.ApplySnapshotChunkResponse_RETRY,
	...
	for index := uint32(0); index < snapshot.Chunks; index++ {
		...
		require.NoError(t, err)
		require.Equal(t, &abci.ApplySnapshotChunkResponse{
			Result: abci.ApplySnapshotChunkResponse_ACCEPT,
		}, respApply)
	}
