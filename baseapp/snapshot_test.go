...
func TestABCI_SnapshotWithPruning(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			// Query 1
			res, err := suite.baseApp.Query(context.TODO(), &abci.RequestQuery{Path: fmt.Sprintf("/store/%s/key", storeKey2), Data: []byte("0"), Height: lastExistingHeight})
			require.NoError(t, err)
			require.NotNil(t, res, "height: %d", lastExistingHeight)
			require.NotNil(t, res.Value, "height: %d", lastExistingHeight)

			// Query 2
			res, err = suite.baseApp.Query(context.TODO(), &abci.RequestQuery{Path: fmt.Sprintf("/store/%s/key", storeKey2), Data: []byte("0"), Height: lastExistingHeight - 1})
			require.NoError(t, err)
