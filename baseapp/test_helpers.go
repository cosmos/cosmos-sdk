...
func (app *BaseApp) SimWriteState() {
	app.finalizeBlockState.MultiStore.Write()
}
...
func (app *BaseApp) NewContextLegacy(isCheckTx bool, header cmtproto.Header) sdk.Context {
	if isCheckTx {
		return sdk.NewContext(app.getState(execModeCheck).MultiStore, header, true, app.logger).
			WithMinGasPrices(app.minGasPrices)
	}

	return sdk.NewContext(app.getState(execModeFinalize).MultiStore, header, false, app.logger)
}
