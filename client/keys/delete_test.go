...
func Test_runDeleteCmd(t *testing.T) {
	...
	if !runningUnattended {
		kb, err := keybase.NewKeyring(sdk.KeyringServiceName(), viper.GetString(flags.FlagKeyringBackend), viper.GetString(flags.FlagHome), mockIn)
	...
	// Now
	kb, err := keybase.NewKeyring(sdk.KeyringServiceName(), viper.GetString(flags.FlagKeyringBackend), kbHome, mockIn)
	require.NoError(t, err)
	if runningUnattended {
