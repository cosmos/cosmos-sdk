...
func MigrateCommand(config *sdk.Config) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "migrate",
...
func runMigrateCmd(config *sdk.Config) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		// instantiate legacy keybase
		rootDir := viper.GetString(flags.FlagHome)
		legacykb, err := NewKeyBaseFromDir(rootDir, config)
		if err != nil {
			return err
		}
		...
		keyringServiceName := sdk.NewDefaultConfig().GetKeyringServiceName()
