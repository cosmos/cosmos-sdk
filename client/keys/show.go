...
func runShowCmd(cmd *cobra.Command, args []string) (err error) {
	...
	if len(args) == 1 {
		...
		info = keybase.NewMultiInfo(defaultMultiSigKeyName, multikey)
	}
	...
	if isShowDevice {
		...
		if info.GetType() != keybase.TypeLedger {
...
func getBechKeyOut(bechPrefix string) (bechKeyOutFn, error) {
	...
	case sdk.PrefixAccount:
		return keybase.Bech32KeyOutput, nil
	case sdk.PrefixValidator:
		return keybase.Bech32ValKeyOutput, nil
	case sdk.PrefixConsensus:
		return keybase.Bech32ConsKeyOutput, nil
	}
