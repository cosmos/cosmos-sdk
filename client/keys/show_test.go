...
func Test_multiSigKey_Properties(t *testing.T) {
	...
	tmp := keybase.NewMultiInfo("myMultisig", pk)

	require.Equal(t, "myMultisig", tmp.GetName())
	require.Equal(t, keybase.TypeMulti, tmp.GetType())
...
func Test_getBechKeyOut(t *testing.T) {
	...
	tests := []struct {
		...
		{"empty", args{""}, nil, true},
		{"wrong", args{"???"}, nil, true},
		{"acc", args{sdk.PrefixAccount}, keybase.Bech32KeyOutput, false},
		{"val", args{sdk.PrefixValidator}, keybase.Bech32ValKeyOutput, false},
		{"cons", args{sdk.PrefixConsensus}, keybase.Bech32ConsKeyOutput, false},
	}
	for _, tt := range tests {
		tt := tt
		t.Run(tt.name, func(t *testing.T) {
