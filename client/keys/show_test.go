...
func Test_showKeysCmd(t *testing.T) {
	cmd := ShowKeysCmd(sdk.NewDefaultConfig())
...
func Test_runShowCmd(t *testing.T) {
	...
	cmd := ShowKeysCmd(config)
	mockIn, _, _ := tests.ApplyMockIO(cmd)
	require.EqualError(t, runShowCmd(config)(cmd, []string{"invalid"}),
		"The specified item could not be found in the keyring")
	require.EqualError(t, runShowCmd(config)(cmd, []string{"invalid1", "invalid2"}), "The specified item could not be found in the keyring")
	...
	require.EqualError(t, runShowCmd(config)(cmd, []string{fakeKeyName1}), "invalid Bech32 prefix encoding provided: ")
	...
	require.NoError(t, runShowCmd(config)(cmd, []string{fakeKeyName1}))
	...
	require.EqualError(t, runShowCmd(config)(cmd, []string{fakeKeyName1, fakeKeyName2}), "threshold must be a positive integer")
	...
	err = runShowCmd(config)(cmd, []string{fakeKeyName1, fakeKeyName2})
	require.NoError(t, err)
