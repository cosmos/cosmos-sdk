...
// QueryABCI performs a query to a CometBFT node with the provide QueryRequest.
// It returns the ResultQuery obtained from the query. The height used to perform
// the query is the QueryRequest Height if it is non-zero, otherwise the context
// height is used.
