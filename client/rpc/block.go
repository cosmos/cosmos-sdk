...
func getBlock(cliCtx context.Context, height *int64) ([]byte, error) {
	// get the node
...
func GetChainHeight(cliCtx context.Context) (int64, error) {
	node, err := cliCtx.GetNode()
	if err != nil {
		return -1, err
	}
...
func printBlock(cmd *cobra.Command, args []string) error {
	...
	output, err := getBlock(context.NewContext(), height)
	if err != nil {
		return err
	}
...
func BlockRequestHandlerFn(cliCtx context.Context) http.HandlerFunc {
...
func LatestBlockRequestHandlerFn(cliCtx context.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
