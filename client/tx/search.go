...
func SearchTxs(cliCtx context.CLIContext, cdc *codec.Codec, tags []string, page, limit int) ([]sdk.ResponseFormat, error) {
	if len(tags) == 0 {
...
func FormatTxResults(cdc *codec.Codec, res []*ctypes.ResultTx) ([]sdk.ResponseFormat, error) {
	var err error
	out := make([]sdk.ResponseFormat, len(res))
	for i := range res {
