...
// defined.
func (ctx txContext) BroadcastTx(txBytes []byte) (res *sdk.TxResponse, err error) {
...
func (ctx txContext) BroadcastTxSync(txBytes []byte) (*sdk.TxResponse, error) {
...
func (ctx txContext) BroadcastTxAsync(txBytes []byte) (*sdk.TxResponse, error) {
	node, err := ctx.GetNode()
	if err != nil {
		return nil, err
	}
...
// ServiceBroadcast is a helper function to broadcast a Tx with the correct gRPC types
// from the tx service. Calls `clientCtx.BroadcastTx` under the hood.
func ServiceBroadcast(_ context.Context, clientCtx txContext, req *tx.BroadcastTxRequest) (*tx.BroadcastTxResponse, error) {
