...
func appStateFn(r *rand.Rand, accs []simulation.Account) json.RawMessage {
	...
	stakeGenesis := stake.GenesisState{
		...
		Params: stake.Params{
			UnbondingTime: time.Duration(between(r, 60, 60*60*24*3*2)) * time.Second,
	...
	slashingGenesis := slashing.GenesisState{
		Params: slashing.Params{
			...
			DoubleSignUnbondDuration: time.Duration(between(r, 60, 60*60*24)) * time.Second,
			SignedBlocksWindow:       int64(between(r, 10, 1000)),
			DowntimeUnbondDuration:   time.Duration(between(r, 60, 60*60*24)) * time.Second,
