...
func (l LookupMap[K, V]) validateGenesis(reader io.Reader) error {
	return l.doDecodeJSON(reader, func(key K, value V) error {
		return nil
	})
}

func (l LookupMap[K, V]) importGenesis(ctx context.Context, reader io.Reader) error {
	return l.doDecodeJSON(reader, func(key K, value V) error {
		return l.Set(ctx, key, value)
	})
}

func (l LookupMap[K, V]) doDecodeJSON(reader io.Reader, onEntry func(key K, value V) error) error {
	decoder := json.NewDecoder(reader)
	token, err := decoder.Token()
	if err != nil {
		return err
	...
	for decoder.More() {
		...
		key, err := l.kc.DecodeJSON(mapEntry.Key)
		if err != nil {
			return err
		}

		value, err := l.vc.DecodeJSON(mapEntry.Value)
		if err != nil {
			return err
		}
...
func (l LookupMap[K, V]) defaultGenesis(writer io.Writer) error {
	_, err := io.WriteString(writer, `[]`)
	return err
}
