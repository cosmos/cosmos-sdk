...
import (
	...
	_ "github.com/cosmos/cosmos-sdk/contrib/api/group/v1"
...
func (s *CLITestSuite) SetupSuite() {
	...
	out, err := clitestutil.ExecTestCLICmd(s.clientCtx, cli.MsgCreateGroupCmd(),
		append(
			[]string{
				val.Address.String(),
				validMetadata,
...
func (s *CLITestSuite) TestTxUpdateGroupAdmin() {
	...
	cmd := cli.MsgUpdateGroupAdminCmd()
	cmd.SetOut(io.Discard)
	...
	for i := range 2 {
	  ...
	  "address": "%s",
		...
		validMembersFile := testutil.WriteToNewTempFile(s.T(), validMembers)
		out, err := clitestutil.ExecTestCLICmd(s.baseCtx, cli.MsgCreateGroupCmd(),
			append(
				[]string{
					accounts[0].Address.String(),
					validMetadata,
...
func (s *CLITestSuite) TestTxUpdateGroupMetadata() {
	...
	cmd := cli.MsgUpdateGroupMetadataCmd()
	cmd.SetOut(io.Discard)
...
func (s *CLITestSuite) TestTxUpdateGroupMembers() {
	...
	cmd := cli.MsgUpdateGroupMembersCmd()
	cmd.SetOut(io.Discard)
...
func (s *CLITestSuite) TestTxCreateGroupWithPolicy() {
	...
	cmd := cli.MsgCreateGroupWithPolicyCmd()
	cmd.SetOut(io.Discard)
	...
	}{
		{
			...
			append(
				[]string{
					...
					fmt.Sprintf("--%s=%v", cli.FlagGroupPolicyAsAdmin, false),
				},
				s.commonFlags...,
			),
			"",
			fmt.Sprintf("%s %s %s %s %s --%s=%v", accounts[0].Address.String(), validMetadata, validMetadata, validMembersFile.Name(), thresholdDecisionPolicyFile.Name(), cli.FlagGroupPolicyAsAdmin, false),
		},
		{
			...
			append(
				[]string{
					...
					validMembersFile.Name(),
					thresholdDecisionPolicyFile.Name(),
					fmt.Sprintf("--%s=%v", cli.FlagGroupPolicyAsAdmin, true),
				},
				s.commonFlags...,
			),
			"",
			fmt.Sprintf("%s %s %s %s %s --%s=%v", accounts[0].Address.String(), validMetadata, validMetadata, validMembersFile.Name(), thresholdDecisionPolicyFile.Name(), cli.FlagGroupPolicyAsAdmin, true),
		},
		{
			"with amino-json",
			append(
			...
			"",
			fmt.Sprintf("%s %s %s %s %s --%s=%v --%s=%s", accounts[0].Address.String(), validMetadata, validMetadata, validMembersFile.Name(), thresholdDecisionPolicyFile.Name(), cli.FlagGroupPolicyAsAdmin, false, flags.FlagSignMode, flags.SignModeLegacyAminoJSON),
		},
		{
			"invalid members weight",
			append(
			...
			"weight must be positive",
			fmt.Sprintf("%s %s %s %s %s --%s=%v", accounts[0].Address.String(), validMetadata, validMetadata, invalidMembersWeightFile.Name(), thresholdDecisionPolicyFile.Name(), cli.FlagGroupPolicyAsAdmin, false),
		},
	}
	for _, tc := range testCases {
		s.Run(tc.name, func() {
...
func (s *CLITestSuite) TestTxUpdateGroupPolicyAdmin() {
	...
	cmd := cli.MsgUpdateGroupPolicyAdminCmd()
	cmd.SetOut(io.Discard)
...
func (s *CLITestSuite) TestTxUpdateGroupPolicyMetadata() {
	...
	cmd := cli.MsgUpdateGroupPolicyMetadataCmd()
	cmd.SetOut(io.Discard)
	...
	testCases := []struct {
		...
		},
		{
			...
			append(
				[]string{
					proposalFile.Name(),
					fmt.Sprintf("--%s=try", cli.FlagExec),
				},
				s.commonFlags...,
			),
			fmt.Sprintf("%s --%s=try", proposalFile.Name(), cli.FlagExec),
			"",
		},
		{
...
func (s *CLITestSuite) TestTxVote() {
	...
	testCases := []struct {
		...
		},
		{
			...
			fmt.Sprintf("%s %s %s %s --%s=try", ids[1], accounts[0].Address.String(), "VOTE_OPTION_YES", "", cli.FlagExec),
			"",
		},
		{
			"with amino-json",
...
func (s *CLITestSuite) TestTxWithdrawProposal() {
	...
	cmd := cli.MsgWithdrawProposalCmd()
	cmd.SetOut(io.Discard)
