...
func TestNewKeyring(t *testing.T) {
	...
	info, _, err := kr.NewMnemonic("foo", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestKeyManagementKeyRing(t *testing.T) {
	...
	pub1 := priv1.PubKey()
	i, err = kb.SavePubKey(o1, pub1, salgo.Ed25519Type)
	require.Nil(t, err)
...
func TestSignVerifyKeyRingWithLedger(t *testing.T) {
	...
	i1, err := kb.SaveLedgerKey("key", privkey.Secp256k1, "cosmos", 118, 0, 0)
	if err != nil {
	...
	localInfo, _, err := kb.NewMnemonic("test", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestExportImportKeyRing(t *testing.T) {
	...
	info, _, err := kb.NewMnemonic("john", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestSeedPhraseKeyRing(t *testing.T) {
	...
	newInfo, err := kb.NewAccount(n2, mnemonic, DefaultBIP39Passphrase, hdPath, salgo.Secp256k1)
	require.NoError(t, err)
...
func TestKeyringKeybaseExportImportPrivKey(t *testing.T) {
	...
	_, _, err = kb.NewMnemonic("john", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestInMemoryLanguage(t *testing.T) {
	kb := NewInMemory()
	_, _, err := kb.NewMnemonic("something", Japanese, types.FullFundraiserPath, privkey.Secp256k1)
	require.Error(t, err)
...
func TestInMemoryCreateAccountInvalidMnemonic(t *testing.T) {
	...
	_, err := kb.NewAccount(
		...
		"", hd.CreateHDPath(118, 0, 0).String(), privkey.Secp256k1)
	require.Error(t, err)
...
func TestInMemoryCreateLedger(t *testing.T) {
	...
	ledger, err := kb.SaveLedgerKey("some_account", privkey.Secp256k1, "cosmos", 118, 3, 1)
...
func TestInMemoryKeyManagement(t *testing.T) {
	...
	priv1 := ed25519.GenPrivKey()
	pub1 := priv1.PubKey()
	i, err = cstore.SavePubKey(o1, pub1, salgo.Ed25519Type)
	require.Nil(t, err)
	require.Equal(t, pub1, i.GetPubKey())
	require.Equal(t, o1, i.GetName())
	iOffline := i.(*offlineInfo)
	require.Equal(t, salgo.Ed25519Type, iOffline.GetAlgo())
	keyS, err = cstore.List()
	require.NoError(t, err)
	require.Equal(t, 2, len(keyS))
...
func TestInMemoryExportImport(t *testing.T) {
	...
	info, _, err := cstore.NewMnemonic("john", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
	require.Equal(t, info.GetName(), "john")
...
func TestKeyChain_ShouldFailWhenAddingSameGeneratedAccount(t *testing.T) {
	...
	_, seed, err := kr.NewMnemonic("test", English, "", privkey.Secp256k1)
	require.NoError(t, err)

	require.NoError(t, kr.Delete("test"))

	path := hd.CreateHDPath(118, 0, 0).String()
	_, err = kr.NewAccount("test1", seed, "", path, privkey.Secp256k1)
	require.NoError(t, err)

	// Creating another account with different uid but same seed should fail due to have same pub address
	_, err = kr.NewAccount("test2", seed, "", path, privkey.Secp256k1)
	require.Error(t, err)
}
...
func TestAltKeyring_List(t *testing.T) {
	...
	_, _, err = keyring.NewMnemonic(uid1, English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
	_, _, err = keyring.NewMnemonic(uid2, English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
	_, _, err = keyring.NewMnemonic(uid3, English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestAltKeyring_NewAccount(t *testing.T) {
	...
	info, err := keyring.NewAccount(uid, mnemonic, DefaultBIP39Passphrase, sdk.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestAltKeyring_SaveLedgerKey(t *testing.T) {
	...
	ledger, err := keyring.SaveLedgerKey("some_account", privkey.Secp256k1, "cosmos", 118, 3, 1)
	if err != nil {
		require.Equal(t, "ledger nano S: support for ledger devices is not available in this executable", err.Error())
		t.Skip("ledger nano S: support for ledger devices is not available in this executable")
		return
...
func TestAltKeyring_Get(t *testing.T) {
	...
	uid := someKey
	mnemonic, _, err := keyring.NewMnemonic(uid, English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestAltKeyring_Delete(t *testing.T) {
	...
	uid := someKey
	_, _, err = keyring.NewMnemonic(uid, English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestAltKeyring_SavePubKey(t *testing.T) {
	...
	info, err := keyring.SavePubKey(key, pub, privkey.Secp256k1.Name())
	require.Nil(t, err)
...
func TestAltKeyring_SaveMultisig(t *testing.T) {
	...
	mnemonic1, _, err := keyring.NewMnemonic("key1", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
	mnemonic2, _, err := keyring.NewMnemonic("key2", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
...
func TestAltKeyring_ConstructorSupportedAlgos(t *testing.T) {
	...
	_, _, err = keyring.NewMnemonic("test", English, types.FullFundraiserPath, privkey.Secp256k1)
	require.NoError(t, err)
