...
//
// In combination with the bip39 package in go-crypto this package provides
// the functionality for deriving keys using a
// BIP 44 HD path, or, more general, by passing a BIP 32 path.
//
// In particular, this package (together with bip39) provides all necessary
// functionality to derive keys from
...
func NewParamsFromPath(path string) (*BIP44Params, error) {
	...
	if !isHardened(spl[1]) || !isHardened(spl[2]) {
		return nil, fmt.Errorf(
			"second and third field in path must be hardened (ie. contain the suffix ', " +
				"got %v and %v", spl[1], spl[2])
	}
	if isHardened(spl[3]) || isHardened(spl[4]) {
		return nil, fmt.Errorf(
			"fourth and fifth field in path must not be hardened (ie. not contain the suffix ', " +
				"got %v and %v", spl[3], spl[4])
	}
...
// The fixed parameters (purpose', coin_type', and change) are
// determined by what was used in the fundraiser.
...
// DerivePrivateKeyForPath derives the private key by following the
// BIP 32/44 path from privKeyBytes, using the given chainCode.
func DerivePrivateKeyForPath(privKeyBytes [32]byte, chainCode [32]byte,
	path string) ([32]byte, error) {
	data := privKeyBytes
	...
	if n != 32 || len(data) != 32 {
		return [32]byte{}, fmt.Errorf(
			"expected a (secp256k1) key of length 32, got length: %v", len(data))
	}
...
//  - https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki
func derivePrivateKey(privKeyBytes [32]byte, chainCode [32]byte,
	index uint32, harden bool) ([32]byte, [32]byte) {
	var data []byte
	if harden {
