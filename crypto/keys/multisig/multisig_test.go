...
func TestMultiSigMigration(t *testing.T) {
	...
	sig := legacytx.StdSignature{PubKey: pkSet[1], Signature: sigs[1].(*signing.SingleSignatureData).Signature} //nolint:staticcheck // SA1019: legacytx.StdSignature is deprecated, but we're using it for comparison purposes here in a test.
