  ...
  (JNIEnv* env, jclass classObject, jobject byteBufferObject, jlong ctx_l)
{
  ...
  int ret = cosmos_secp256k1_ecdsa_sign(ctx, sig, data, secKey, NULL, NULL );

  unsigned char outputSer[72];
  size_t outputLen = 72;

  if( ret ) {
    int ret2 = cosmos_secp256k1_ecdsa_signature_serialize_der(ctx,outputSer, &outputLen, sig ); (void)ret2;
  }
  ...
  (JNIEnv* env, jclass classObject, jobject byteBufferObject, jlong ctx_l)
{
  ...
  if( ret ) {
    int ret2 = cosmos_secp256k1_ec_pubkey_serialize(ctx,outputSer, &outputLen, &pubkey,SECP256K1_EC_UNCOMPRESSED );(void)ret2;
  }
  ...
  (JNIEnv* env, jclass classObject, jobject byteBufferObject, jlong ctx_l)
{
  ...
  int ret = cosmos_ secp256k1_ec_privkey_tweak_add(ctx, privkey, tweak);
...
/*  secp256k1_pubkey* pubkey = (secp256k1_pubkey*) (*env)->GetDirectBufferAddress(env, byteBufferObject);*/
  ...
  if( ret ) {
    ret = cosmos_secp256k1_ec_pubkey_tweak_add(ctx, &pubkey, tweak);
  }
  ...
  (JNIEnv* env, jclass classObject, jobject byteBufferObject, jlong ctx_l, jint publen)
{
  ...
  if ( ret ) {
    ret = cosmos_secp256k1_ec_pubkey_tweak_mul(ctx, &pubkey, tweak);
  }
