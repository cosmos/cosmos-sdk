...
type PrivKey struct {
	MiniSecretKey []byte `protobuf:"bytes,1,opt,name=miniSecretKey,proto3" json:"miniSecretKey,omitempty"`
}
...
func (*PrivKey) Descriptor() ([]byte, []int) {
	return fileDescriptor_daddabaf35039fbd, []int{1}
}
...
func (m *PrivKey) GetMiniSecretKey() []byte {
	if m != nil {
		return m.MiniSecretKey
	}
	return nil
}
...
var fileDescriptor_daddabaf35039fbd = []byte{
	...
	0x0b, 0xe4, 0x19, 0x94, 0xf4, 0xb9, 0xd8, 0x03, 0x8a, 0x32, 0xcb, 0x40, 0x4a, 0x54, 0xb8, 0x78,
	0x73, 0x33, 0xf3, 0x32, 0x83, 0x53, 0x93, 0x8b, 0x52, 0x4b, 0xbc, 0xe1, 0x8a, 0x51, 0x05, 0x9d,
	...
	0xa3, 0x34, 0x49, 0x2f, 0x39, 0x3f, 0x57, 0x1f, 0xe6, 0x0d, 0x30, 0xa5, 0x5b, 0x9c, 0x92, 0x0d,
	0xf3, 0x11, 0xc8, 0x27, 0x30, 0x6f, 0x25, 0xb1, 0x81, 0x5d, 0x69, 0x0c, 0x08, 0x00, 0x00, 0xff,
	0xff, 0xd9, 0x26, 0xe1, 0xf0, 0xf6, 0x00, 0x00, 0x00,
}
...
func (m *PrivKey) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.MiniSecretKey) > 0 {
		i -= len(m.MiniSecretKey)
		copy(dAtA[i:], m.MiniSecretKey)
		i = encodeVarintKeys(dAtA, i, uint64(len(m.MiniSecretKey)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
...
func (m *PrivKey) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MiniSecretKey", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.MiniSecretKey = append(m.MiniSecretKey[:0], dAtA[iNdEx:postIndex]...)
			if m.MiniSecretKey == nil {
				m.MiniSecretKey = []byte{}
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
