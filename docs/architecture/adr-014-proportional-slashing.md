...
In Proof of Stake-based chains, centralization of consensus power amongst a small set of validators can cause harm to the network due to increased risk of censorship, liveness failure, fork attacks, etc. However, while this centralization causes a negative externality to the network, it is not directly felt by the delegators delegating to already large validators. We would like a way to pass on the negative externality cost of centralization onto those large validators and their delegators.
...
Now, if someone splits a validator of 10% into two validators of 5% each which both fault in the same time frame, they will both get slashed at the sum 10% amount.
...
We then will iterate over all the SlashEvents in the queue, adding their `ValidatorVotingPercent` to calculate the new percent to slash all validators in the queue, using the "Square of Sum of Roots" formula introduced above.
