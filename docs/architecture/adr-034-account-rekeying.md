...
This is possible because the Cosmos SDK `BaseAccount` stores the public key for an account in its state, rather than assuming that the public key is included in the transaction (whether explicitly or implicitly through the signature), as is the case with other blockchains such as Bitcoin and Ethereum.  Because the public key is stored on the chain, it is okay for the public key not to hash to the address of an account, as the address is not pertinent to the signature checking process.
