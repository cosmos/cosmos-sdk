...
- `cdc` is the [codec](../core/encoding.md) used to marshall and unmarshall a struct to and from `[]byte`. The `cdc` can be any of `codec.BinaryCodec`, `codec.JSONCodec` or `codec.Codec` based on your requirements. It can be either a proto or amino codec as long as they implement these interfaces.
...
and it will go through the following steps:
