...
Based on the definition of the `Msg` service, Protobuf generates a `MsgServer` interface. It is the role of the module developer to implement this interfaces, by implementing the state transition logic that should happen upon receival of each `Msg`. As an example, here is the generated `MsgServer` interfaces for `x/bank`:
