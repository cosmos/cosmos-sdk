...
In order to successfully upgrade your existing modules, your `AppModule`s must implement the function `ConsensusVersion() uint64`. The versions should be hard coded by the module developer. Consensus versions serve as a state-breaking versions of app modules, and will be incremented when the module is upgraded. The initial version *MUST* be set to 1.
