...
Each full-node (running  CometBFT ) that receives a `Tx` sends an [ABCI message](https://docs.tendermint.com/master/spec/abci/abci.html#messages),
...
execute the consensus algorithm, such as [ CometBFT  BFT](https://docs.tendermint.com/master/spec/consensus/consensus.html#terms),
