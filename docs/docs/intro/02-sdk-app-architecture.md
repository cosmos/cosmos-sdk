...
Thanks to the Cosmos SDK, developers just have to define the state machine, and [*CometBFT*](https://docs.tendermint.com/master/introduction/what-is-tendermint.html) will handle replication over the network for them.
...
[CometBFT](https://docs.tendermint.com/v0.34/introduction/what-is-tendermint.html) is an application-agnostic engine that is responsible for handling the *networking* and *consensus* layers of a blockchain. In practice, this means that CometBFT is responsible for propagating and ordering transaction bytes. CometBFT Core relies on an eponymous Byzantine-Fault-Tolerant (BFT) algorithm to reach consensus on the order of transactions.
...
CometBFT passes transactions to the application through an interface called the [ABCI](https://docs.tendermint.com/master/spec/abci/), which the application must implement.
...
* `DeliverTx`: When a [valid block](https://docs.tendermint.com/v0.34/spec/blockchain/blockchain.html#validation) is received by CometBFT Core, each transaction in the block is passed to the application via `DeliverTx` in order to be processed. It is during this stage that the state transitions occur. The `AnteHandler` executes again, along with the actual [`Msg` service](../building-modules/03-msg-services.md) RPC for each message in the transaction.
...
Find a more detailed view of the ABCI methods from the [CometBFT docs](https://docs.tendermint.com/v0.34/introduction/what-is-tendermint.html#abci-overview).
