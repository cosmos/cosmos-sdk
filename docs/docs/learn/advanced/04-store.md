...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/store.go#L17-L20
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/store.go#L285-L317
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/store.go#L34-L38
```

The `Committer` is an interface that defines methods to persist changes to disk:

```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/store.go#L22-L32
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/store.go#L115-L147
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/store.go#L155-L225
```

As for concrete implementation, the [`rootMulti.Store`] is the go-to implementation of the `CommitMultiStore` interface.

```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/rootmulti/store.go#L56-L82
```
...
Whenever the `rootMulti.Store` needs to be branched, a [`cachemulti.Store`](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/cachemulti/store.go) is used.

```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/cachemulti/store.go#L20-L34
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/store.go#L227-L264
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/x/bank/keeper/view.go#L121-L137
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/iavl/store.go#L36-L41
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/dbadapter/store.go#L13-L16
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/transient/store.go#L16-L19
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/x/params/types/subspace.go#L22-L32
```

Transient stores are typically accessed via the [`context`](./02-context.md) via the `TransientStore()` method:

```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/types/context.go#L347-L350
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/cachekv/store.go#L26-L36
```
...
Cosmos SDK applications use [`gas`](../beginner/04-gas-fees.md) to track resources usage and prevent spam. [`GasKv.Store`](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/gaskv/store.go) is a `KVStore` wrapper that enables automatic gas consumption each time a read or write to the store is made. It is the solution of choice to track storage usage in Cosmos SDK applications.

```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/gaskv/store.go#L11-L17
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/gas.go#L219-L228
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/types/context.go#L342-L345
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/types/gas.go#L230-L241
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/tracekv/store.go#L20-L43
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/prefix/store.go#L15-L21
```
...
Additional information about state streaming configuration can be found in the [store/streaming/README.md](https://github.com/cosmos/cosmos-sdk/tree/v0.53.0-rc.2/store/streaming).

```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.53.0-rc.2/store/listenkv/store.go#L11-L18
```
