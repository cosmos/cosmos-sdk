...
- `genesis.go`: The module's genesis related business logic that uses SDK's context(e.g. `InitGenesis` and `ExportGenesis`).
While, genesis types that don't require context are defined in `internal/types`(e.g. `ValidateGenesis`). 
