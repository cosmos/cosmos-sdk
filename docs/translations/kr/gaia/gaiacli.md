...
```bash
gaiacli tx send ... --gas-prices=0.000001atom
```
...
```bash
gaiacli tx gov deposit <proposal_id(프로포절 ID)> <200atom(금액)> \
  --from=<name(트랜잭션을 발생시킬 키/계정 이름)> \
  --chain-id=<chain_id(체인 아이디)>
```
...
```bash
gaiacli tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 10atom \
