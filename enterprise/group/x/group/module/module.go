...
func (am AppModule) WeightedOperationsX(weights simsx.WeightSource, reg simsx.Registry) {
	s := simulation2.NewSharedState()
	// note: using old keys for backwards compatibility
	reg.Add(weights.Get("msg_create_group", 100), simulation2.MsgCreateGroupFactory())
	reg.Add(weights.Get("msg_update_group_admin", 5), simulation2.MsgUpdateGroupAdminFactory(am.keeper, s))
	reg.Add(weights.Get("msg_update_group_metadata", 5), simulation2.MsgUpdateGroupMetadataFactory(am.keeper, s))
	reg.Add(weights.Get("msg_update_group_members", 5), simulation2.MsgUpdateGroupMembersFactory(am.keeper, s))
	reg.Add(weights.Get("msg_create_group_account", 50), simulation2.MsgCreateGroupPolicyFactory(am.keeper, s))
	reg.Add(weights.Get("msg_create_group_with_policy", 50), simulation2.MsgCreateGroupWithPolicyFactory())
	reg.Add(weights.Get("msg_update_group_account_admin", 5), simulation2.MsgUpdateGroupPolicyAdminFactory(am.keeper, s))
	reg.Add(weights.Get("msg_update_group_account_decision_policy", 5), simulation2.MsgUpdateGroupPolicyDecisionPolicyFactory(am.keeper, s))
	reg.Add(weights.Get("msg_update_group_account_metadata", 5), simulation2.MsgUpdateGroupPolicyMetadataFactory(am.keeper, s))
	reg.Add(weights.Get("msg_submit_proposal", 2*90), simulation2.MsgSubmitProposalFactory(am.keeper, s))
	reg.Add(weights.Get("msg_withdraw_proposal", 20), simulation2.MsgWithdrawProposalFactory(am.keeper, s))
	reg.Add(weights.Get("msg_vote", 90), simulation2.MsgVoteFactory(am.keeper, s))
