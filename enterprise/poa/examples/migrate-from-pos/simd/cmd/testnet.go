...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		peerID := fmt.Sprintf("%s@%s:%d", nodeIDs[i], ip, p2pPortStart+portOffset)
		...
		// make the first node the admin
		if i == 0 {
			admin = addr
		}
		pubkeyAny, err := types.NewAnyWithValue(valPubKeys[i])
		if err != nil {
			return err
		}
	...
	if err := initGenFiles(clientCtx, mm, args.chainID, genAccounts, genBalances, genFiles, validators, admin); err != nil {
		return err
	}
...
func initGenFiles(
	...
	genFiles []string, validators []poatypes.Validator, admin sdk.AccAddress,
) error {
	...
	poaGenBz, ok := appGenState[poatypes.ModuleName]
	if !ok {
		return fmt.Errorf("no genesis state found for module %s", poatypes.ModuleName)
	}
	clientCtx.Codec.MustUnmarshalJSON(poaGenBz, &poaGenesis)
	poaGenesis.Validators = validators
	poaGenesis.Params.Admin = admin.String()
	appGenState[poatypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&poaGenesis)
