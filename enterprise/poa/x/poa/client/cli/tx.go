...
func NewTxCommand(pubkeyFactory map[string]func(codec.Codec, []byte) *codectypes.Any) *cobra.Command {
	...
	txCmd.AddCommand(
		...
		NewRotateConsPubKeyCmd(pubkeyFactory),
...
// NewRotateConsPubKeyCmd returns a CLI command for rotating a validator's consensus pubkey.
// The --from signer becomes the sender, so the same command serves operator self-rotation
// and admin override.
func NewRotateConsPubKeyCmd(pubkeyFactory map[string]func(codec.Codec, []byte) *codectypes.Any) *cobra.Command {
	const flagOperator = "operator-address"
	cmd := &cobra.Command{
		// Use matches the name autocli derives from the RotateConsPubKey RPC so the
		// custom command replaces the autogenerated one instead of duplicating it.
		Use:     "rotate-cons-pub-key",
		Aliases: []string{"rotate-cons-pubkey"},
		Short:   "Rotate a validator's consensus public key",
		Long: "Rotate the consensus public key of the validator at --operator-address to the supplied new key. " +
			"The --from signer must be either that operator (self-rotation) or the module admin (override).",
		Args: cobra.ExactArgs(2),
		Example: fmt.Sprintf(
			"%s tx poa rotate-cons-pub-key pubkey_base64 pubkey_type --operator-address cosmos1... --from mykey",
			version.AppName,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			...
			pubkeyBase64, pubkeyType := args[0], args[1]

			pubkeyBz, err := base64.StdEncoding.DecodeString(pubkeyBase64)
			if err != nil {
				return fmt.Errorf("failed to decode pubkey_base64: %w", err)
			}
	...
	cmd.Flags().String(flagOperator, "", "operator address of the validator whose key is rotated")
	_ = cmd.MarkFlagRequired(flagOperator)
	flags.AddTxFlagsToCmd(cmd)
