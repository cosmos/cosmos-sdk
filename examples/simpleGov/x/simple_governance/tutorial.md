...
- **Application:** Responsible for managing and replicating the application's foundational state machine and processing transactions.

The *networking* layer makes sure that each validator node receives transactions. The *consensus* layer makes sure that each node agrees on the same transactions to modify their original state. As for the *application* layer, it communicates with the other layers to process transactions. Given a transaction and the status of the application's state, the application will change to a new state. In Bitcoin for example, the application state is a ledger or list of balances for each account (in reality, it's a list of UTXO, short for Unspent Transaction Output, but let's call them balances for the sake of simplicity), and the transactions modify the application's state by changing these list of balances. In the case of Ethereum, the application is a virtual machine. Each transaction goes through this virtual machine and modifies the application state according to the parameters of the smart contract. 
...
Tendermint was designed to address these issues devlopers faced with Ethereum and Bitcoin. The goal of Tendermint and the Cosmos Network is to provide the *networking* and *consensus* layers of a blockchain as a generic engine to power any application developers want to build. With Tendermint, developers only have to focus on the *application* layer, thereby saving hundreds of hours of work and costly development set-ups. For reference, Tendermint Core, the Byzantine fault-tolerant (BFT) consensus engine, is the combination of networking and consensus as discussed above.

Tendermint connects the blockchain engine, Tendermint Core, (*networking* and *consensus* layers) to the application layer via a communication protocol called Application-BlockChain Interface [ABCI](https://github.com/tendermint/abci). Developers only have to implement a few messages to build an ABCI-enabled application that communications with and runs on top of the Tendermint Core engine. ABCI is language agnostic, meaning that developers can build the application part of their blockchain in any programming language. Building on top of the Tendermint Core engine also provides the following benefits:
...
- **Performance.** Tendermint Core is a state-of-the-art blockchain consensus engine able to handle a large number of transactions quickly. A block time on Tendermint Core can be as low as one second and can process thousands of transactions in that time period.
- **Instant finality.** A property of the Tendermint Core consensus algorithm is instant finality, so users can be sure their transactions are finalized as soon as a block is created. This is because forks can't happen, unless
at least a 1/3 majority of Cosmos Validators have signed duplicitously (equaling 2/3 so over 100%), an indicator that validators are actinging maliciously (Byzantine). 
- **Security.** Tendermint Core's consensus is not only fault tolerant, it’s optimally Byzantine fault-tolerant (BFT), with accountability. As explained above, if the blockchain forks, there is a way to determine liability.
...
In terms of its design, the SDK optimizes flexibility and security. The framework is designed around a modular execution stack which allows applications to mix and match elements as desired, or complete customization. In addition, all modules are sandboxed for greater application security.
...
- **Capabilities:** The SDK is inspired by capabilities-based security, and informed by years of wrestling with blockchain state-machines. Most developers will need to access other 3rd party modules when building their own modules. Given that the Cosmos-SDK is an open framework and that we assume that some of those modules may be malicious, we designed the SDK using object-capabilities (ocaps) based principles. In practice, this means that instead of having each module keep an access control list for other modules, each module implements keepers (WHAT IS A KEEPER) that can be passed to other modules to grant a pre-defined set of capabilities. For example, if an instance of module A's keepers is passed to module B, the latter will be able to call a restricted set of module A's functions. The capabilities of each keeper are defined by the module's developer, and it's the developer's job to understand and audit the safety of foreign code from 3rd party modules based on the capabilities they are passing into each 3rd party module. For a deeper look at capabilities, you can read this article (WHAT ARTICLE).
...
- `DeliverTx`: Ask the application to process the transaction and change the state-machine.
...
For this tutorial, we will code a **simple governance application**, accompagnied by a **simple governance module**. It will allow us to explain most of the basic notions required to build a functioning application on the Cosmos Network. Note that this is not the governance module used for the Cosmos Hub. A much more [advanced governance module](https://github.com/cosmos/cosmos-sdk/tree/develop/x/gov) will be used instead.
