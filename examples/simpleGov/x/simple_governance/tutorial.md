...
The first method we have to create is the constructor.

```go
func NewKeeper(proposalStoreKey sdk.StoreKey, ck bank.CoinKeeper, sm stake.KeeperRead, codespace sdk.CodespaceType) Keeper
```

This method is called from the main `app.go` file to instanciate a new `Keeper`. A similar method exits for `KeeperRead`. 
...
Handlers implement the core logic of the state-machine. When a 
