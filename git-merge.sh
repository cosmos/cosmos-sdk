...
git_diff_add_rm() {
    ...
    while read -r stat rest; do
      case $stat in
      D) force git rm -f "$rest" || true ;;
      R*)
        file=$(echo "$rest" | cut -d' ' -f1)
        renamed=$(echo "$rest" | cut -d' ' -f2)
        force git rm -f "$file" || true
        force git add -f "$renamed"
        ;;
      A | M) force git add -f "$rest" ;;
      esac
    done
}
...
Summary:
--------
1. Merge upstream diff size: $(wc -l < "$WORKSPACE/diff_upstream_diff.patch") lines
