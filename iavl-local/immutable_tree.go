...
// GetWithSource returns the value for the key and which IAVL layer produced the result.
// This is for debugging only.
func (t *ImmutableTree) GetWithSource(key []byte) ([]byte, IAVLGetSource, error) {
	if t.root == nil {
	...
	if !t.skipFastStorageUpgrade {
		...
		if fastNode.GetVersionLastUpdatedAt() <= t.version {
			...
			return fastNode.GetValue(), IAVLSourceFastNodeDB, nil
		}

		_, result, err := t.root.get(t, key)
		return result, IAVLSourceFastNodeStale, err
	}
