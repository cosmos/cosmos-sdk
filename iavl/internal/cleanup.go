...
type cleanupProc struct {
	...
	pendingOrphanQueue []markOrphansReq // orphans waiting to be processed
...
//		}
//	}
func (cp *cleanupProc) MarkOrphans(version uint32, orphans []*NodePointer) {
	req := markOrphansReq{
		version: version,
		orphans: orphans,
	}
	...
	cp.pendingOrphanQueue = append(cp.pendingOrphanQueue, req)
}
