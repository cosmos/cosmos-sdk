...
func nextIndex(node Node, key []byte) (int64, error) {
	...
	if node.IsLeaf() {
		switch bytes.Compare(nKey.UnsafeBytes(), key) {
		case -1:
			return 1, nil
		case 1:
			return 0, nil
	...
	if bytes.Compare(key, nKey.UnsafeBytes()) < 0 {
