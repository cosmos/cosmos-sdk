...
func (t tableGen) getTableInterface() {
	...
	if t.table.PrimaryKey.AutoIncrement {
		...
		t.P("CurrentSequence(ctx ", contextPkg.Ident("Context"), ") (uint64, error)")
	}
...
func (t tableGen) genTableImpl() {
	...
	if t.table.PrimaryKey.AutoIncrement {
		...
		t.P(receiver, "CurrentSequence(ctx ", contextPkg.Ident("Context"), ") (uint64, error) {")
		t.P("return ", receiverVar, ".table.CurrentSequence(ctx)")
		t.P("}")
		t.P()
	}
