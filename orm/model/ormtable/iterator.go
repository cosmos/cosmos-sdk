...
func prefixIterator(iteratorStore kv.ErrReadOnlyStore, backend ReadBackend, index concreteIndex, codec *ormkv.KeyCodec, prefix []interface{}, opts []listinternal.Option) (Iterator, error) {
...
func rangeIterator(iteratorStore kv.ErrReadOnlyStore, reader ReadBackend, index concreteIndex, codec *ormkv.KeyCodec, start, end []interface{}, opts []listinternal.Option) (Iterator, error) {
	options := &listinternal.Options{}
	listinternal.ApplyOptions(options, opts)
	if err := options.Validate(); err != nil {
		return nil, err
