...
func addSanityCheckAndSync(lastBlockPersisted uint64, listener appdata.Listener, mgrOpts IndexingOptions, moduleFilter ModuleFilterConfig) appdata.Listener {
...
func doSyncAndSanityCheck(lastBlockPersisted uint64, data appdata.StartBlockData, listener appdata.Listener, mgrOpts IndexingOptions, moduleFilter ModuleFilterConfig) error {
	if lastBlockPersisted == 0 {
		...
		// we are out of sync and there is existing data, so return an error
