...
verify_attestation() {
	...
	if ! command -v gh >/dev/null 2>&1; then
		echo "WARNING: gh CLI not found; skipping build provenance verification for ${archive_path}" >&2
		echo "Install the GitHub CLI (https://cli.github.com) to verify provenance, or set SKIP_ATTESTATION=1 to silence this warning." >&2
		return 0
	fi
