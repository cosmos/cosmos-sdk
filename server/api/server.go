...
import (
	...
	logwrp "github.com/cosmos/cosmos-sdk/server/log"
...
func (s *Server) Start(ctx context.Context, cfg config.Config) error {
	...
	go func(enableUnsafeCORS bool) {
		...
		if enableUnsafeCORS {
			...
			errCh <- tmrpcserver.Serve(s.listener, allowAllCORS(s.Router), logwrp.CometZeroLogWrapper{Logger: s.logger}, cmtCfg)
		} else {
			errCh <- tmrpcserver.Serve(s.listener, s.Router, logwrp.CometZeroLogWrapper{Logger: s.logger}, cmtCfg)
		}
