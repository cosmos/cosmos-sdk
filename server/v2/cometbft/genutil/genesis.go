...
func NewAppGenesisWithVersion(chainID, appName, version string, appState json.RawMessage) *AppGenesis {
	return &AppGenesis{
		AppName:    appName,
		AppVersion: version,
		ChainID:    chainID,
...
func AppGenesisFromReader(reader io.Reader, appName string) (*AppGenesis, error) {
...
func AppGenesisFromFile(appName, genFile string) (*AppGenesis, error) {
	file, err := os.Open(filepath.Clean(genFile))
	if err != nil {
		return nil, err
	}

	appGenesis, err := AppGenesisFromReader(bufio.NewReader(file), appName)
	if err != nil {
