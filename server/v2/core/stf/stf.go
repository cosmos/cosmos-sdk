...
type STF[T transaction.Tx] interface {
	...
	) (*appmanager.BlockResponse, store.WritableAccountsState, error)
	// Simulate simulates the execution of a transaction over the provided state, with the provided gas limit.
	Simulate(ctx context.Context, state store.ReadonlyAccountsState, gasLimit uint64, tx T) (appmanager.TxResult, store.WritableAccountsState)
	// Query runs the provided query over the provided readonly state.
	Query(ctx context.Context, state store.ReadonlyAccountsState, gasLimit uint64, queryRequest appmanager.Type) (queryResponse appmanager.Type, err error)
	// ValidateTx validates the TX.
	ValidateTx(ctx context.Context, state store.ReadonlyAccountsState, gasLimit uint64, tx T) appmanager.TxResult
}
