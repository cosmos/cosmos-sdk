...
func DoBenchmarkDeepCacheStack(b *testing.B, depth int) {
	b.Helper()
	db := newMemState()
	initialStore := NewStore[store.ReadonlyState](db)
...
func (cs *CacheStack) Reset(initialStore Store[store.ReadonlyState]) {
...
func (cs *CacheStack) Snapshot() int {
	cs.cacheStores = append(cs.cacheStores, NewStore[store.ReadonlyState](cs.CurrentStore()))
