...
## [Unreleased]
* [#26037](https://github.com/cosmos/cosmos-sdk/pull/26037) Remove `GetCommitStore` and `GetCommitKVStore` from the `CommitMultiStore` interface. Deprecate `CommitStore` and `CommitKVStore` interfaces. Remove top-level `store.CommitStore` and `store.CommitKVStore` type aliases from `store/reexport.go`.
