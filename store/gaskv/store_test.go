...
func TestGasKVStoreIterator(t *testing.T) {
	...
	iterator.Next()
	require.Equal(t, types.Gas(15565), meter.GasConsumed())
	kc := iterator.Key()
	require.Equal(t, kc, keyFmt(3))
	vc := iterator.Value()
	require.Equal(t, vc, valFmt(0))
	iterator.Next()
	require.Equal(t, types.Gas(15667), meter.GasConsumed())
	require.False(t, iterator.Valid())
	require.Panics(t, iterator.Next)
	require.Equal(t, types.Gas(15697), meter.GasConsumed())
	require.NoError(t, iterator.Error())
	...
	require.Equal(t, types.Gas(17135), meter.GasConsumed())
}
