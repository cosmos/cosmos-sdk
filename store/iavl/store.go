...
func LoadStore(db dbm.DB, logger log.Logger, key types.StoreKey, id types.CommitID, lazyLoading bool, cacheSize int,
	skipFastStorageUpgrade bool) (types.CommitKVStore, error) {
	return LoadStoreWithInitialVersion(db, logger, key, id, lazyLoading, 0, cacheSize, skipFastStorageUpgrade)
}
...
func LoadStoreWithInitialVersion(db dbm.DB, logger log.Logger, key types.StoreKey, id types.CommitID, lazyLoading bool,
	initialVersion uint64, cacheSize int, skipFastStorageUpgrade bool) (types.CommitKVStore, error) {
	tree, err := iavl.NewMutableTreeWithOpts(db, cacheSize, &iavl.Options{InitialVersion: initialVersion}, skipFastStorageUpgrade)
	if err != nil {
		return nil, err
	}
