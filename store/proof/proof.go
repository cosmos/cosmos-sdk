...
func (op CommitmentOp) Run(args [][]byte) ([][]byte, error) {
	...
	if err != nil {
		return nil, errorsmod.Wrapf(store.ErrInvalidProof, "could not calculate root for proof: %v", err)
	}
	...
	case 0:
		...
		if !absent {
			return nil, errorsmod.Wrapf(store.ErrInvalidProof, "proof did not verify absence of key: %s", string(op.Key))
		}

	case 1:
		// Args is length 1, verify existence of key with value args[0]
		if !ics23.VerifyMembership(op.Spec, root, op.Proof, op.Key, args[0]) {
			return nil, errorsmod.Wrapf(store.ErrInvalidProof, "proof did not verify existence of key %s with given value %x", op.Key, args[0])
		}

	default:
		return nil, errorsmod.Wrapf(store.ErrInvalidProof, "args must be length 0 or 1, got: %d", len(args))
	}
