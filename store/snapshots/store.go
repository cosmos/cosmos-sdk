...
func NewStore(db db.DB, dir string) (*Store, error) {
	if dir == "" {
		return nil, errorsmod.Wrap(storetypes.ErrLogic, "snapshot directory not given")
	}
...
func (s *Store) Delete(height uint64, format uint32) error {
	...
	s.mtx.Unlock()
	if saving {
		return errorsmod.Wrapf(storetypes.ErrConflict,
...
func (s *Store) Save(
	...
	if height == 0 {
		return nil, errorsmod.Wrap(storetypes.ErrLogic, "snapshot height cannot be 0")
	}
	...
	for chunkBody := range chunks {
		...
		if err != nil {
			return nil, errorsmod.Wrapf(err, "failed to create snapshot chunk file %q", path)
		}
		...
		if err != nil {
			return nil, errorsmod.Wrapf(err, "failed to generate snapshot chunk %v", index)
		}
		err = file.Close()
		if err != nil {
			return nil, errorsmod.Wrapf(err, "failed to close snapshot chunk %v", index)
		}
		err = chunkBody.Close()
		if err != nil {
			return nil, errorsmod.Wrapf(err, "failed to close snapshot chunk %v", index)
...
func decodeKey(k []byte) (uint64, uint32, error) {
	if len(k) != 13 {
		return 0, 0, errorsmod.Wrapf(storetypes.ErrLogic, "invalid snapshot key with length %v", len(k))
	}
	if k[0] != keyPrefixSnapshot {
		return 0, 0, errorsmod.Wrapf(storetypes.ErrLogic, "invalid snapshot key prefix %x", k[0])
	}
