...
// Set encodes and sets the argument to the state. It calls Delete if the
// argument is nil.
...
// StoreName returns the mapping's store name.
func (m Mapping) StoreName() string {
	return m.storeKey.Name()
}

// PrefixBytes returns the mapping's prefix bytes.
...
// KeyBytes returns the mapping's key bytes.
