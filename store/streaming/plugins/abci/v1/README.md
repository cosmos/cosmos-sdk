...
See [store/streaming/plugins/proto/abci/v1/grpc.proto](https://github.com/cosmos/cosmos-sdk/blob/main/store/streaming/plugins/proto/abci/v1/grpc.proto) for full details.
...
option go_package = "github.com/cosmos/cosmos-sdk/store/streaming/plugins/abci/v1";
...
```shell
buf export buf.build/cosmos/cosmos-sdk:${commit} --output store/streaming/plugins/proto
```
...
```shell
protoc -I store/streaming/plugins/proto --go_out=plugins=grpc:. store/streaming/plugins/proto/abci/v1/grpc.proto \
  && cp -r github.com/cosmos/cosmos-sdk/store/streaming/* store/streaming \
  && rm -rf github.com 
```

For other languages you'll need to [download](https://github.com/cosmos/cosmos-sdk/blob/main/third_party/proto/README.md)
the CosmosSDK protos and the [store/streaming/plugins/proto/abci/v1/grpc.proto](https://github.com/cosmos/cosmos-sdk/blob/main/store/streaming/plugins/proto/abci/v1/grpc.proto)
...
Boilerplate RPC implementation of the `ABCIListener` interface. ([store/streaming/plugins/abci/v1/grpc.go](grpc.go))
```go
...
...
- stdout plugin
```shell
go build -o store/streaming/plugins/abci/v1/examples/stdout store/streaming/plugins/abci/v1/examples/stdout.go
```

- file plugin (writes to ~/)
```shell
go build -o store/streaming/plugins/abci/v1/examples/file store/streaming/plugins/abci/v1/examples/file.go
```
...
- stdout plugin
```shell
export COSMOS_SDK_ABCI_V1="{path to}/cosmos-sdk/store/streaming/plugins/abci/v1/examples/stdout"
```
- file plugin (writes to ~/)
```shell
export COSMOS_SDK_ABCI_V1="{path to}/cosmos-sdk/store/streaming/plugins/abci/v1/examples/file"
```
