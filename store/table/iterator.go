...
func (i IteratorFunc) LoadNext(_ sdk.KVStore, dest codec.ProtoMarshaler) (RowID, error) {
	return i(dest)
}
...
// The key is the rowID and not any MultiKeyIndex key.
func (i *LimitedIterator) LoadNext(store sdk.KVStore, dest codec.ProtoMarshaler) (RowID, error) {
	if i.remainingCount == 0 {
		return nil, ErrIteratorDone
	}
	i.remainingCount--
	return i.parentIterator.LoadNext(store, dest)
}
...
func First(store sdk.KVStore, it Iterator, dest codec.ProtoMarshaler) (RowID, error) {
	if it == nil {
		return nil, errors.Wrap(ErrArgument, "iterator must not be nil")
	}
	defer it.Close()
	binKey, err := it.LoadNext(store, dest)
	if err != nil {
		return nil, err
	}
	return binKey, nil
...
func Paginate(
	...
	for {
		...
		binKey, err := it.LoadNext(store, modelProto)
		if err != nil {
...
//
func ReadAll(store sdk.KVStore, it Iterator, dest ModelSlicePtr) ([]RowID, error) {
	if it == nil {
		return nil, errors.Wrap(ErrArgument, "iterator must not be nil")
	}
	defer it.Close()
	...
	for {
		...
		binKey, err := it.LoadNext(store, model.Interface().(codec.ProtoMarshaler))
		switch {
		case err == nil:
			tmpSlice = reflect.Append(tmpSlice, val)
