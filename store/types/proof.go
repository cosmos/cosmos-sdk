...
func CommitmentOpDecoder(pop tmmerkle.ProofOp) (merkle.ProofOperator, error) {
	...
	case ProofOpSMTCommitment:
		spec = ics23.SmtSpec
	default:
		return nil, ErrInvalidProof.Wrapf("unexpected ProofOp.Type; got %s, want supported ics23 subtypes 'ProofOpSimpleMerkleCommitment', 'ProofOpIAVLCommitment', or 'ProofOpSMTCommitment'", pop.Type)
	}
...
func (op CommitmentOp) Run(args [][]byte) ([][]byte, error) {
	...
	if err != nil {
		return nil, ErrInvalidProof.Wrapf("could not calculate root for proof: %v", err)
	}
	...
	case 0:
		...
		if !absent {
			return nil, ErrInvalidProof.Wrapf("proof did not verify absence of key: %s", string(op.Key))
		}

	case 1:
		// Args is length 1, verify existence of key with value args[0]
		if !ics23.VerifyMembership(op.Spec, root, op.Proof, op.Key, args[0]) {
			return nil, ErrInvalidProof.Wrapf("proof did not verify existence of key %s with given value %x", op.Key, args[0])
		}
	default:
		return nil, ErrInvalidProof.Wrapf("args must be length 0 or 1, got: %d", len(args))
	}
