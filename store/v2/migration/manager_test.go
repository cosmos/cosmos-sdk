...
func setupMigrationManager(t *testing.T, noCommitStore bool) (*Manager, *commitment.CommitStore) {
	...
	commitStore, err := commitment.NewCommitStore(multiTrees, db, nil, coretesting.NewNopLogger())
	...
	newCommitStore, err := commitment.NewCommitStore(multiTrees1, db1, nil, coretesting.NewNopLogger()) // for store/v2
	require.NoError(t, err)
	if noCommitStore {
		newCommitStore = nil
	}
