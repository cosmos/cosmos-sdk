// This package provides concrete implementations of the store/v2 "RootStore" types, including
// CommitRootStore, CacheRootStore, and BasicRootStore (as read-only stores at past versions).
//
// Substores are declared as part of a schema within StoreOptions.
// The schema cannot be changed once a CommitRootStore is initialized, and changes to the schema must be done
...
// If the RootStore is configured to use a separate DBConnection for StateCommitmentDB, it will store the
...
// RootStore (using the Merkle map proof spec (TendermintSpec)).
