...
func (rs *Store) Restore(height uint64, format uint32, chunks <-chan io.ReadCloser, ready chan<- struct{}) error {
	...
	for {
		...
		case *storetypes.SnapshotItem_Schema:
			...
			ret.StoreSchema[string(sKey)] = types.StoreType(sValue[0])
			ret.reserved = append(ret.reserved, string(sKey)) // assume iter yields keys sorted

		case *storetypes.SnapshotItem_Store:
			// set the new snapshot store schema to root-store
			rs.schema = ret.StoreSchema
			sstore, err = rs.getSubstore(item.Store.Name)
			if err != nil {
				return sdkerrors.Wrap(err, fmt.Sprintf("error while getting the substore for key %s", item.Store.Name))
			}

		case *storetypes.SnapshotItem_KV:
			// update the key/value SMT.Store
			sstore.Set(item.KV.Key, item.KV.Value)
		}
	}
...
func (rs *Store) Snapshot(height uint64, format uint32) (<-chan io.ReadCloser, error) {
	...
	go func() {
		...
		for key := range vs.schema {
			...
			iter := getSubstore.Iterator(nil, nil)
