...
func (s *RootStoreTestSuite) newStoreWithPruneConfig(config *store.PruningOption) {
	...
	sc, err := commitment.NewCommitStore(multiTrees, dbm.NewMemDB(), nil, noopLog)
	s.Require().NoError(err)
...
func (s *RootStoreTestSuite) TestMultiStore_PruningRestart() {
	...
	sc, err := commitment.NewCommitStore(map[string]commitment.Tree{testStoreKey: tree}, mdb2, nil, noopLog)
	s.Require().NoError(err)
...
func (s *RootStoreTestSuite) TestMultiStoreRestart() {
	...
	sc, err := commitment.NewCommitStore(multiTrees, mdb2, nil, noopLog)
	s.Require().NoError(err)
