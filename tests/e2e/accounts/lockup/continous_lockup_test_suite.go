...
func (s *E2ETestSuite) TestContinuousLockingAccount() {
	...
	_, accountAddr, err := app.AccountsKeeper.Init(ctx, types.CONTINUOUS_LOCKING_ACCOUNT, accOwner, &types.MsgInitLockupAccount{
		Owner:     ownerAddrStr,
...
func (s *E2ETestSuite) TestContinuousLockingAccountClawbackEnable() {
	t := s.T()
	app := setupApp(t)
	currentTime := time.Now()
	ctx := sdk.NewContext(app.CommitMultiStore(), false, app.Logger()).WithHeaderInfo(header.Info{
	...
	t.Run("ok - execute clawback", func(t *testing.T) {
		msg := &types.MsgClawback{
			Admin:  ownerAddrStr,
			Denoms: []string{"stake"},
		}
		...
		balance := app.BankKeeper.GetBalance(ctx, accOwner, "stake")
		// should be 1000 - 200 (vested token) = 800
