...
func (s *E2ETestSuite) TestPeriodicLockingAccount() {
	...
	_, accountAddr, err := app.AccountsKeeper.Init(ctx, types.PERIODIC_LOCKING_ACCOUNT, accOwner, &types.MsgInitPeriodicLockingAccount{
		Owner:     ownerAddrStr,
...
func (s *E2ETestSuite) TestPeriodicLockingAccountClawbackEnable() {
	t := s.T()
	app := setupApp(t)
	currentTime := time.Now()
	ctx := sdk.NewContext(app.CommitMultiStore(), false, app.Logger()).WithHeaderInfo(header.Info{
	...
	t.Run("ok - execute clawback", func(t *testing.T) {
		...
		// should be 1500 - 500 (sent token) - 500 (withdraw token) = 500
		require.True(t, balance.Amount.Equal(math.NewInt(500)))
	})
}
