...
func (s *WithdrawAllTestSuite) TestNewWithdrawAllRewardsGenerateOnly() {
	...
	_, err = clitestutil.MsgSendExec(
		...
		val.Address,
		newAddr,
		sdk.NewCoins(sdk.NewCoin(s.cfg.BondDenom, sdkmath.NewInt(2000))), addresscodec.NewBech32Codec("cosmos"), fmt.Sprintf("--%s=true", flags.FlagSkipConfirmation),
	...
	args := []string{
		...
		sdk.NewCoin(s.cfg.BondDenom, sdkmath.NewInt(500)).String(),
