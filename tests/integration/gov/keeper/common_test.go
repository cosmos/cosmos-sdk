...
func createValidators(t *testing.T, f *fixture, powers []int64) ([]sdk.AccAddress, []sdk.ValAddress) {
	...
	err = f.stakingKeeper.SetValidatorByConsAddr(ctx, val1)
	assert.NilError(t, err)
	err = f.stakingKeeper.SetValidatorByConsAddr(ctx, val2)
	assert.NilError(t, err)
	err = f.stakingKeeper.SetValidatorByConsAddr(ctx, val3)
	assert.NilError(t, err)
