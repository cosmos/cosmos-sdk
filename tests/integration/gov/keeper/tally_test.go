...
func TestTallyNoOneVotes(t *testing.T) {
	...
	tp := TestProposal
	proposal, err := f.govKeeper.SubmitProposal(
		f.ctx,
		tp,
		"",
		"test",
		"description",
		sdk.AccAddress("cosmos1ghekyjucln7y67ntx7cf27m9dpuxxemn4c8g4r"),
		v1.ProposalType_PROPOSAL_TYPE_STANDARD,
	)
	assert.NilError(t, err)
	proposalID := proposal.Id
	proposal.Status = v1.StatusVotingPeriod
	err = f.govKeeper.Proposals.Set(f.ctx, proposal.Id, proposal)
	assert.NilError(t, err)
	proposal, ok := f.govKeeper.Proposals.Get(f.ctx, proposalID)
	assert.Assert(t, ok)
	passes, burnDeposits, tallyResults, _ := f.govKeeper.Tally(f.ctx, proposal)
...
func TestTallyNoQuorum(t *testing.T) {
	...
	proposal, ok := f.govKeeper.Proposals.Get(f.ctx, proposalID)
	assert.Assert(t, ok)
	passes, burnDeposits, _, _ := f.govKeeper.Tally(f.ctx, proposal)
	assert.Assert(t, passes == false)
	assert.Assert(t, burnDeposits == false)
}
...
func TestTallyOnlyValidators51No(t *testing.T) {
	...
	assert.NilError(
		...
		f.govKeeper.AddVote(
			...
			proposalID,
			valAccAddrs[1],
			v1.NewNonSplitVoteOption(v1.OptionNo),
			"",
		),
	)
...
func TestTallyOnlyValidatorsVetoed(t *testing.T) {
	...
	assert.NilError(
		...
		f.govKeeper.AddVote(
			...
			proposalID,
			valAccAddrs[2],
			v1.NewNonSplitVoteOption(v1.OptionNoWithVeto),
			"",
		),
	)
...
func TestTallyOnlyValidatorsAbstainPasses(t *testing.T) {
	...
	assert.NilError(
		...
		f.govKeeper.AddVote(
			...
			proposalID,
			valAccAddrs[0],
			v1.NewNonSplitVoteOption(v1.OptionAbstain),
			"",
		),
	)
	assert.NilError(
...
func TestTallyDelgatorOverride(t *testing.T) {
	...
	delTokens := f.stakingKeeper.TokensFromConsensusPower(f.ctx, 30)
	val1, found := f.stakingKeeper.GetValidator(f.ctx, valAddrs[0])
...
func TestTallyDelgatorMultipleOverride(t *testing.T) {
	...
	delTokens := f.stakingKeeper.TokensFromConsensusPower(f.ctx, 10)
	val1, found := f.stakingKeeper.GetValidator(f.ctx, vals[0])
	assert.Assert(t, found)
	val2, found := f.stakingKeeper.GetValidator(f.ctx, vals[1])
	assert.Assert(t, found)
	_, err := f.stakingKeeper.Delegate(
		f.ctx,
		addrs[3],
...
func TestTallyJailedValidator(t *testing.T) {
	...
	delTokens := f.stakingKeeper.TokensFromConsensusPower(f.ctx, 10)
	val2, found := f.stakingKeeper.GetValidator(f.ctx, valAddrs[1])
	assert.Assert(t, found)
	val3, found := f.stakingKeeper.GetValidator(f.ctx, valAddrs[2])
	assert.Assert(t, found)
	...
	assert.NilError(t, err)
	assert.NilError(t, f.stakingKeeper.Jail(f.ctx, sdk.ConsAddress(consAddr)))
...
func TestTallyValidatorMultipleDelegations(t *testing.T) {
	...
	expectedYes := f.stakingKeeper.TokensFromConsensusPower(f.ctx, 30)
	expectedAbstain := f.stakingKeeper.TokensFromConsensusPower(f.ctx, 0)
	expectedNo := f.stakingKeeper.TokensFromConsensusPower(f.ctx, 10)
	expectedNoWithVeto := f.stakingKeeper.TokensFromConsensusPower(f.ctx, 0)
	expectedTallyResult := v1.NewTallyResult(
		expectedYes,
		expectedAbstain,
		expectedNo,
		expectedNoWithVeto,
