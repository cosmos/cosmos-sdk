...
func initFixture(tb testing.TB) *fixture {
	...
	integrationApp := integration.NewIntegrationApp(logger, keys, cdc, map[string]appmodule.AppModule{
		...
		err = slashingKeeper.Params.Set(ctx, testutil.TestParams())
		assert.NilError(tb, err)
		addrDels = simtestutil.AddTestAddrsIncremental(bankKeeper, stakingKeeper, ctx, 6, stakingKeeper.TokensFromConsensusPower(ctx, 200))
		...
		err = slashingKeeper.ValidatorSigningInfo.Set(ctx, sdk.ConsAddress(addrDels[0]), info1)
		assert.NilError(tb, err)
		err = slashingKeeper.ValidatorSigningInfo.Set(ctx, sdk.ConsAddress(addrDels[1]), info2)
		assert.NilError(tb, err)
...
func TestUnJailNotBonded(t *testing.T) {
	...
	err := f.app.RunArbitraryCode(func(ctx sdk.Context) error {
		p, err := f.stakingKeeper.GetParams(ctx)
		assert.NilError(t, err)
		p.MaxValidators = 5
		assert.NilError(t, f.stakingKeeper.SetParams(ctx, p))
		...
		vals, err := f.stakingKeeper.GetAllValidators(ctx)
		assert.NilError(t, err)
		assert.Equal(t, len(vals), 6)

		_, err = f.stakingKeeper.EndBlocker(ctx)
		assert.NilError(t, err)
		// f.ctx = f.ctx.WithBlockHeight(ctx.BlockHeight() + 1)
	...
	err = f.app.RunArbitraryCode(func(ctx sdk.Context) error {
		...
		tstaking.DelegateWithPower(sdk.AccAddress(addr), addr, 100)

		_, err = f.stakingKeeper.EndBlocker(ctx)
		assert.NilError(t, err)

		// tstaking.CheckValidator(addr, stakingtypes.Bonded, true)
