...
func TestRollback(t *testing.T) {
	testCases := []struct {
		...
		{"disable offline rollback", false},
		{"enable offline rollback", true},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// rollback 5 blocks
			target := ver0 + 5
			rs := app.CommitMultiStore()
			require.NoError(t, rs.RollbackToVersionWithMode(target, tc.offlineRollback))
			if tc.offlineRollback {
				// assume loadLatestVersion on restart after offline rollback
