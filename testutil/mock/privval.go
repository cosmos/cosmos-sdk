...
func (pv PV) SignOracleVote(chainID string, oracleVote *oracleproto.GossipedVotes, sigPrefix []byte) error {
	signBytes := cmttypes.OracleVoteSignBytes(chainID, oracleVote)
	sig, err := pv.PrivKey.Sign(signBytes)
	if err != nil {
		return err
	}
