...
func TestDataWatcher(t *testing.T) {
	...
	pollWatcher := NewFilePollWatcher(ctx, eh, filename, time.Millisecond*100)
	dataWatcher := NewDataWatcher[TestData](ctx, eh, pollWatcher, func(contents []byte) (TestData, error) {
		var data TestData
	...
	go func() {
		...
		err = os.WriteFile(filename, []byte("unexpected content - should be ignored"), 0o600)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		err = os.WriteFile(filename, bz, 0o600)
		require.NoError(t, err)
