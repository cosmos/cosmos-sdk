...
func TestMigrateFatalImportWarningsAbortBeforeMutations(t *testing.T) {
	tmpDir := t.TempDir()
...
`
	...
	groupSrc := `package main

import "cosmossdk.io/x/group"

func main() {}
`
	if err := os.WriteFile(filepath.Join(tmpDir, "main.go"), []byte(groupSrc), 0o600); err != nil {
		t.Fatalf("write main.go: %v", err)
	}
...
//go:build !app_v1
`
	if err := os.WriteFile(filepath.Join(tmpDir, "app_di.go"), []byte(diSrc), 0o600); err != nil {
		t.Fatalf("write app_di.go: %v", err)
	}

	args := MigrateArgs{
		ImportWarnings: []ImportWarning{
			{ImportPrefix: "cosmossdk.io/x/group", Message: "group removed", Fatal: true},
		},
		FileRemovals: []FileRemoval{
			{FileName: "app_di.go", ContainsMustMatch: "!app_v1"},
		},
		GoModUpdates: GoModUpdate{
			"github.com/cosmos/cosmos-sdk": "v0.54.0",
		},
	...
	err := Migrate(tmpDir, args)
	if err == nil || !strings.Contains(err.Error(), "fatal import warnings") {
		t.Fatalf("expected fatal import warning error, got: %v", err)
	}

	if _, err := os.Stat(filepath.Join(tmpDir, "app_di.go")); err != nil {
		t.Fatalf("expected app_di.go to remain after fatal abort, got: %v", err)
	}
	...
	if !strings.Contains(string(modContent), "github.com/cosmos/cosmos-sdk v0.53.6") {
		t.Fatalf("expected go.mod to remain unchanged after fatal abort, got:\n%s", string(modContent))
	}
}
