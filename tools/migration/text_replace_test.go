...
func TestApplyTextReplacements(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "RequiresContains gates rewrite",
			content: "anteHandler, err := NewAnteHandler(\n\tHandlerOptions{}\n)\n",
			replacements: []TextReplacement{
				{
			...
			wantModified: false,
			wantContains: []string{"anteHandler, err := NewAnteHandler("},
		},
		{
			name:    "RequiresContains allows rewrite when guard matches",
			content: "anteHandler, err := NewAnteHandler(\n\tHandlerOptions{}\n)\n\t_ = &app.CircuitKeeper\n",
			replacements: []TextReplacement{
				{
					Old:              "anteHandler, err := NewAnteHandler(",
			...
			wantModified: true,
			wantContains: []string{"anteHandler, err := ante.NewAnteHandler("},
			wantMissing:  []string{"anteHandler, err := NewAnteHandler("},
		},
		{
