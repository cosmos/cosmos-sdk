...
Most simapp-shaped changes are automated, but custom chains still need manual
review around forked `replace` directives, historical upgrade files, custom ante
handlers, and non-standard keeper wiring.

Fatal preflight findings are not part of that manual review loop. If `x/group`
is detected, the tool should stop before deleting files or rewriting `go.mod`.
...
- Bumps `cosmossdk.io/x/tx` while preserving its vanity import path
- Bumps CometBFT to v0.39.0-beta.3
- Replaces `cosmossdk.io/log` with `cosmossdk.io/log/v2`
- Removes selected `cosmossdk.io/x/*` vanity URL modules that were folded into the SDK monorepo

### Import Rewrites
- `cosmossdk.io/log` → `cosmossdk.io/log/v2`
- `cosmossdk.io/x/{feegrant,evidence,upgrade}` → `github.com/cosmos/cosmos-sdk/x/*`
...
Audit any preserved non-local `replace` directives before assuming the dependency graph is valid on v54.
If the target follows an older simapp-shaped monorepo layout, compare the
migrated output against the current v54 reference file layout before treating
lingering build errors as isolated rewrite bugs. In practice that often means
auditing local `replace` directives and stale app wiring files together.
