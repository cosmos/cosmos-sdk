...
func dropStaleReplaceModules(goModPath string, modulePaths ...string) error {
	...
	for _, rep := range mod.Replace {
		for _, modulePath := range modulePaths {
			...
			if !isLocalReplaceTarget(rep.New.Path) {
				log.Warn().Msgf(
					"preserving non-local replace for %s => %s; audit fork/version compatibility manually",
					rep.Old.Path,
					rep.New.Path,
				)
				continue
...
func isLocalReplaceTarget(path string) bool {
