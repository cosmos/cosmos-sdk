...
func (s *SystemUnderTest) StartNodes(t *testing.T, nodeIDs ...int) error {
	...
	for _, nodeID := range nodeIDs {
		...
		args := []string{
			...
			// override the config.toml value: configs generated by legacy binaries
			// may share one pprof port and CometBFT >= v0.39.4 fails startup when
			// the pprof port is taken
...
func (s *SystemUnderTest) startNodesAsync(t *testing.T, xargs ...string) {
	t.Helper()
	s.withEachNodeHome(func(i int, home string) {
		// per-node pprof port: node configs may share one (e.g. written by a
		// legacy binary's testnet init) and CometBFT >= v0.39.4 fails startup
		// on a pprof bind conflict
...
func (s *SystemUnderTest) AddFullnode(t *testing.T, beforeStart ...func(nodeNumber int, nodePath string)) Node {
	...
	args = []string{
		...
		// the config.toml is copied from node0; override its pprof address as
		// CometBFT >= v0.39.4 fails startup when the pprof port is taken
