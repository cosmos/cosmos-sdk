...
func (s *contextTestSuite) TestContextWithCustom() {
	...
	// test inner context
	newContext := context.WithValue(ctx.Context(), struct{}{}, "value") //nolint: // this is fine for testing
...
func (s *contextTestSuite) TestUnwrapSDKContext() {
	...
	ctx = context.WithValue(sdkCtx, struct{}{}, "bar") //nolint: // this is fine for testing
	sdkCtx2 = types.UnwrapSDKContext(ctx)
	s.Require().Equal(sdkCtx, sdkCtx2)
}
