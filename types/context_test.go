...
func (s *contextTestSuite) TestContextWithCustom() {
	...
	// test inner context
	newContext := context.WithValue(ctx.Context(), struct{}{}, "value") //nolint:staticcheck // using context.WithValue for testing purposes
...
func (s *contextTestSuite) TestUnwrapSDKContext() {
	...
	ctx = context.WithValue(sdkCtx, struct{}{}, "bar") //nolint:staticcheck // using context.WithValue for testing purposes
	sdkCtx2 = types.UnwrapSDKContext(ctx)
	s.Require().Equal(sdkCtx, sdkCtx2)
}
