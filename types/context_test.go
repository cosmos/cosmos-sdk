...
func (s *contextTestSuite) TestContextWithCustom() {
	...
	// test inner context
	newContext := context.WithValue(ctx.Context(), "key", "value") //nolint:revive,staticcheck // we would like to use the strings, and this is a test
...
func (s *contextTestSuite) TestUnwrapSDKContext() {
	...
	ctx = context.WithValue(sdkCtx, "foo", "bar") //nolint:golint,staticcheck,revive // we would like to use the strings, and this is a test
	sdkCtx2 = types.UnwrapSDKContext(ctx)
	s.Require().Equal(sdkCtx, sdkCtx2)
}
