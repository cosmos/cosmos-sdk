...
func NewDecCoin(denom string, amount Uint) DecCoin {
	validateDenom(denom)
...
func NewDecCoinFromDec(denom string, amount Dec) DecCoin {
	...
	if amount.LT(ZeroDec()) {
		panic(fmt.Sprintf("negative decimal coin amount: %v", amount))
	}
...
func NewDecCoinFromCoin(coin Coin) DecCoin {
	if coin.Amount.LT(ZeroUint()) {
		panic(fmt.Sprintf("negative decimal coin amount: %v", coin.Amount))
	}
	if strings.ToLower(coin.Denom) != coin.Denom {
		panic(fmt.Sprintf("denom cannot contain upper case characters: %s", coin.Denom))
	}

	return DecCoin{
		Denom:  coin.Denom,
		Amount: NewDecFromUint(coin.Amount),
	}
}
...
func NewInt64DecCoin(denom string, amount uint64) DecCoin {
	return NewDecCoin(denom, NewUint(amount))
}
...
func (coin DecCoin) TruncateDecimal() (Coin, DecCoin) {
	truncated := coin.Amount.TruncateUint()
	change := coin.Amount.Sub(NewDecFromUint(truncated))
