...
import (
	...
	testutil_helper "github.com/cosmos/cosmos-sdk/x/bank/client/testutil"
...
func (s *paginationTestSuite) TestPagination() {
	...
	s.Require().NoError(testutil_helper.FundAccount(app.BankKeeper, ctx, addr1, balances))
...
func ExamplePaginate() {
	...
	acc1 := app.AccountKeeper.NewAccountWithAddress(ctx, addr1)
	app.AccountKeeper.SetAccount(ctx, acc1)
	err := testutil_helper.FundAccount(app.BankKeeper, ctx, addr1, balances)
