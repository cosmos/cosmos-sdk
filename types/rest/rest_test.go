...
func TestParseQueryHeight(t *testing.T) {
	...
	tests := []struct {
		...
		{"no height", req0, httptest.NewRecorder(), context.Context{}, emptyHeight, true},
		{"height", req1, httptest.NewRecorder(), context.Context{}, height, true},
		{"invalid height", req2, httptest.NewRecorder(), context.Context{}, emptyHeight, false},
		{"negative height", req3, httptest.NewRecorder(), context.Context{}, emptyHeight, false},
	}
	for _, tt := range tests {
		tt := tt
		t.Run(tt.name, func(t *testing.T) {
...
func TestPostProcessResponseBare(t *testing.T) {
	...
	ctx = context.Context{Indent: true}.WithCodec(codec.New())
	w = httptest.NewRecorder()
	data := struct {
		X int    `json:"x"`
		S string `json:"s"`
  ...
  "x": "10"
	...
	ctx = context.Context{Indent: false}.WithCodec(codec.New())
	w = httptest.NewRecorder()
	data = struct {
		X int    `json:"x"`
		S string `json:"s"`
...
func runPostProcessResponse(t *testing.T, ctx context.Context, obj interface{}, expectedBody []byte, indent bool) {
	if indent {
		ctx.Indent = indent
	}
