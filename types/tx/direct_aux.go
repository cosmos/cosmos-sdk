...
func (s *SignDocDirectAux) ValidateBasic() error {
	if len(s.BodyBytes) == 0 {
		return errorstypes.ErrInvalidRequest.Wrap("body bytes cannot be empty")
	}

	if s.PublicKey == nil {
		return errorstypes.ErrInvalidPubKey.Wrap("public key cannot be empty")
	}

	if s.Tip != nil {
		if s.Tip.Tipper == "" {
			return errorstypes.ErrInvalidRequest.Wrap("tipper cannot be empty")
		}
	}
...
func (a *AuxSignerData) ValidateBasic() error {
	if a.Address == "" {
		return errorstypes.ErrInvalidRequest.Wrapf("address cannot be empty")
	}
	...
	if len(a.Sig) == 0 {
		return errorstypes.ErrNoSignatures.Wrap("signature cannot be empty")
	}
...
func (a *AuxSignerData) GetSignatureV2() (signing.SignatureV2, error) {
	...
	if !ok {
		return signing.SignatureV2{}, errorstypes.ErrInvalidType.Wrapf("expected %T, got %T", (cryptotypes.PubKey)(nil), pk)
	}
