...
func RegisterInitHandler[
	...
	router.handler = func(ctx context.Context, env appmodule.Environment, initRequest ProtoMsg) (initResponse ProtoMsg, err error) {
		concrete, ok := initRequest.(ProtoReq)
		if !ok {
			return nil, fmt.Errorf("%w: wanted %s, got %T", errInvalidMessage, reqName, initRequest)
		}
		return handler(ctx, env, concrete)
	}
...
func RegisterExecuteHandler[
	...
	router.handlers[reqName] = func(ctx context.Context, env appmodule.Environment, executeRequest ProtoMsg) (executeResponse ProtoMsg, err error) {
