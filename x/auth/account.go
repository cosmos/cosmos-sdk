...
func (bva *BaseVestingAccount) trackDelegation(vestingCoins, amount sdk.Coins) {
	...
	for _, coin := range amount {
		...
		if !x.IsZero() {
			...
			bva.DelegatedVesting = bva.DelegatedVesting.Add(sdk.NewCoins(xCoin))
		}

		if !y.IsZero() {
			yCoin := sdk.NewCoin(coin.Denom, y)
			bva.DelegatedFree = bva.DelegatedFree.Add(sdk.NewCoins(yCoin))
		}

		bva.Coins = bva.Coins.Sub(sdk.NewCoins(coin))
	}
}
