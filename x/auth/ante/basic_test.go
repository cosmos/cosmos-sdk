...
func TestValidateBasic(t *testing.T) {
	...
	invalidTx, err := suite.CreateTestTx(suite.ctx, privs, accNums, accSeqs, suite.ctx.HeaderInfo().ChainID, signing.SignMode_SIGN_MODE_DIRECT)
	...
	validTx, err := suite.CreateTestTx(suite.ctx, privs, accNums, accSeqs, suite.ctx.HeaderInfo().ChainID, signing.SignMode_SIGN_MODE_DIRECT)
	require.NoError(t, err)
