...
func (spkd SetPubKeyDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for i, pk := range pubkeys {
		...
		if !simulate && !bytes.Equal(pk.Address(), signers[i]) {
			return ctx, errorstypes.ErrInvalidPubKey.Wrapf("pubKey does not match signer address %s with signer index: %d", signers[i], i)
		}
		...
		if err != nil {
			return ctx, errorstypes.ErrInvalidPubKey.Wrap(err.Error())
		}
...
func (svd SigVerificationDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	if len(sigs) != len(signerAddrs) {
		return ctx, errorstypes.ErrUnauthorized.Wrapf("invalid number of signer;  expected: %d, got %d", len(signerAddrs), len(sigs))
	}
	...
	for i, sig := range sigs {
		...
		if !simulate && pubKey == nil {
			return ctx, errorstypes.ErrInvalidPubKey.Wrap("pubkey on account is not set")
		}

		// Check account sequence number.
		if sig.Sequence != acc.GetSequence() {
			return ctx, errorstypes.ErrWrongSequence.Wrapf("account sequence mismatch, expected %d, got %d", acc.GetSequence(), sig.Sequence)
		}
		...
		if !simulate && !ctx.IsReCheckTx() {
			...
			if err != nil {
				...
				return ctx, errorstypes.ErrUnauthorized.Wrap(errMsg)
...
func (vscd ValidateSigCountDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	sigTx, ok := tx.(authsigning.SigVerifiableTx)
	if !ok {
		return ctx, errorstypes.ErrTxDecode.Wrap("Tx must be a sigTx")
	}
	...
	for _, pk := range pubKeys {
		...
		if uint64(sigCount) > params.TxSigLimit {
			return ctx, errorstypes.ErrTooManySignatures.Wrapf("signatures: %d, limit: %d", sigCount, params.TxSigLimit)
		}
	}
...
func DefaultSigVerificationGasConsumer(
	...
	case *ed25519.PubKey:
		...
		return errorstypes.ErrInvalidPubKey.Wrap("ED25519 public keys are unsupported")
...
func GetSignerAcc(ctx sdk.Context, ak AccountKeeper, addr sdk.AccAddress) (types.AccountI, error) {
	...
	return nil, errorstypes.ErrUnknownAddress.Wrapf("account %s does not exist", addr)
}
...
func signatureDataToBz(data signing.SignatureData) ([][]byte, error) {
	...
	case *signing.MultiSignatureData:
		...
		return sigs, nil
	default:
		return nil, errorstypes.ErrInvalidType.Wrapf("unexpected signature data type %T", data)
	}
}
