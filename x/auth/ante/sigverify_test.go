...
func runSigDecorators(t *testing.T, params types.Params, _ bool, privs ...cryptotypes.PrivKey) (storetypes.Gas, error) {
	...
	spkd := ante.NewSetPubKeyDecorator(suite.accountKeeper)
	svd := ante.NewSigVerificationDecorator(suite.accountKeeper, suite.clientCtx.TxConfig.SignModeHandler(), ante.DefaultSigVerificationGasConsumer, false)
	antehandler := sdk.ChainAnteDecorators(spkd, svd)
...
func TestAnteHandlerChecks(t *testing.T) {
	...
	sigVerificationDecorator := ante.NewSigVerificationDecorator(suite.accountKeeper, anteTxConfig.SignModeHandler(), ante.DefaultSigVerificationGasConsumer, true)
