...
func checkTxFeeWithValidatorMinGasPrices(ctx sdk.Context, tx sdk.Tx) (sdk.Coins, int64, error) {
	...
	if !ok {
		return nil, 0, errorstypes.ErrTxDecode.Wrap("Tx must be a FeeTx")
	}
	...
	if ctx.IsCheckTx() {
		...
		if !minGasPrices.IsZero() {
			...
			if !feeCoins.IsAnyGTE(requiredFees) {
				return nil, 0, errorstypes.ErrInsufficientFee.Wrapf("insufficient fees; got: %s required: %s", feeCoins, requiredFees)
			}
		}
	}
