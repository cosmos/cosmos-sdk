...
func TestAnteHandlerSigErrors(t *testing.T) {
	...
	module.CheckInvalidTx(t, input.m, ctx, tx, false, sdk.CodeNoSignatures)

	// test num sigs dont match GetSigners
	privs, accNums, seqs = []crypto.PrivKey{priv1}, []uint64{0}, []uint64{0}
	tx = types.NewTestTx(ctx, msgs, privs, accNums, seqs, fee)
	module.CheckInvalidTx(t, input.m, ctx, tx, false, sdk.CodeUnauthorized)

	// test an unrecognized account
	privs, accNums, seqs = []crypto.PrivKey{priv1, priv2, priv3}, []uint64{0, 1, 2}, []uint64{0, 0, 0}
	tx = types.NewTestTx(ctx, msgs, privs, accNums, seqs, fee)
	module.CheckInvalidTx(t, input.m, ctx, tx, false, sdk.CodeUnknownAddress)
...
func TestAnteHandlerAccountNumbers(t *testing.T) {
	...
	privs, accnums, seqs := []crypto.PrivKey{priv1}, []uint64{0}, []uint64{0}
	tx = types.NewTestTx(ctx, msgs, privs, accnums, seqs, fee)
	module.CheckValidTx(t, input.m, ctx, tx, false)
...
func TestAnteHandlerMemoGas(t *testing.T) {
	...
	module.CheckInvalidTx(t, input.m, ctx, tx, false, sdk.CodeOutOfGas)
	...
	// memo too large
	fee = NewStdFee(12000, sdk.NewCoins(sdk.NewInt64Coin("atom", 0)))
	tx = types.NewTestTxWithMemo(ctx, []sdk.Msg{msg}, privs, accnums, seqs, fee, strings.Repeat("01234567890", 500))
	module.CheckInvalidTx(t, input.m, ctx, tx, false, sdk.CodeMemoTooLarge)
...
func TestAnteHandlerBadSignBytes(t *testing.T) {
	...
	for _, cs := range cases {
		...
		module.CheckInvalidTx(t, input.m, ctx, tx, false, cs.code)
	}
	...
	tx = types.NewTestTx(ctx, msgs, privs, accnums, seqs, fee)
	module.CheckInvalidTx(t, input.m, ctx, tx, false, sdk.CodeInvalidPubKey)
}
...
func TestAnteHandlerSigLimitExceeded(t *testing.T) {
	...
	tx = types.NewTestTx(ctx, msgs, privs, accnums, seqs, fee)
	module.CheckInvalidTx(t, input.m, ctx, tx, false, sdk.CodeTooManySignatures)
}

// Test custom SignatureVerificationGasConsumer
func TestCustomSignatureVerificationGasConsumer(t *testing.T) {
	// setup
	input := setupTestInputGasConsumer(func(meter sdk.GasMeter, sig []byte, pubkey crypto.PubKey, params Params) sdk.Result {
