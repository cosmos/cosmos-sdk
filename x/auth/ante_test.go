...
func checkInvalidTx(t *testing.T, anteHandler sdk.AnteHandler, ctx sdk.Context, tx sdk.Tx, simulate bool, code sdk.CodeType) {
	...
	require.Equal(t, code, result.Code, fmt.Sprintf("Expected %d, got %d (%v)", code, result.Code, result))
...
func TestAnteHandlerFees(t *testing.T) {
	...
	acc1.SetCoins(sdk.Coins{sdk.NewUint64Coin("atom", 149)})
	input.ak.SetAccount(ctx, acc1)
	checkInvalidTx(t, anteHandler, ctx, tx, false, sdk.CodeInsufficientFunds)

	require.True(t, input.fck.GetCollectedFees(ctx).IsEqual(emptyCoins))
	require.True(t, input.ak.GetAccount(ctx, addr1).GetCoins().AmountOf("atom").Equal(sdk.NewUint(149)))

	acc1.SetCoins(sdk.Coins{sdk.NewUint64Coin("atom", 150)})
	input.ak.SetAccount(ctx, acc1)
	checkValidTx(t, anteHandler, ctx, tx, false)

	require.True(t, input.fck.GetCollectedFees(ctx).IsEqual(sdk.Coins{sdk.NewUint64Coin("atom", 150)}))
	require.True(t, input.ak.GetAccount(ctx, addr1).GetCoins().AmountOf("atom").Equal(sdk.NewUint(0)))
}
...
func TestAnteHandlerMemoGas(t *testing.T) {
	...
	msg := newTestMsg(addr1)
	privs, accnums, seqs := []crypto.PrivKey{priv1}, []uint64{0}, []uint64{0}
	fee := NewStdFee(0, sdk.Coins{sdk.NewUint64Coin("atom", 0)})
...
func TestAnteHandlerBadSignBytes(t *testing.T) {
	...
	fee2.Gas += 100
	fee3 := newStdFee()
	fee3.Amount[0].Amount = fee3.Amount[0].Amount.AddUint64(100)
...
func TestEnsureSufficientMempoolFees(t *testing.T) {
	...
	testCases := []struct {
		...
		{NewStdFee(200000, sdk.Coins{sdk.NewUint64Coin("photino", 5)}), false},
		{NewStdFee(200000, sdk.Coins{sdk.NewUint64Coin("stake", 1)}), false},
		{NewStdFee(200000, sdk.Coins{sdk.NewUint64Coin("stake", 2)}), true},
		{NewStdFee(200000, sdk.Coins{sdk.NewUint64Coin("photino", 10)}), true},
		{
			NewStdFee(
				200000,
				sdk.Coins{
					sdk.NewUint64Coin("photino", 10),
					sdk.NewUint64Coin("stake", 2),
				},
			),
			true,
		},
