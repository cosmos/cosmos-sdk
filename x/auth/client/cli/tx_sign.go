	...
	return func(cmd *cobra.Command, args []string) error {
		...
		for sequence := txFactory.Sequence(); scanner.Scan(); sequence++ {
			...
			if ms == "" {
				...
				_, fromName, _, err := client.GetFromFields(txFactory.Keybase(), from, clientCtx.GenerateOnly, clientCtx.Simulate)
				if err != nil {
				...
				multisigAddr, _, _, err := client.GetFromFields(txFactory.Keybase(), ms, clientCtx.GenerateOnly, clientCtx.Simulate)
				if err != nil {
					return fmt.Errorf("error getting account from keybase: %w", err)
				}
...
func makeSignCmd() func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) (err error) {
		...
		from, _ := cmd.Flags().GetString(flags.FlagFrom)
		_, fromName, _, err := client.GetFromFields(txF.Keybase(), from, clientCtx.GenerateOnly, clientCtx.Simulate)
		if err != nil {
			return fmt.Errorf("error getting account from keybase: %w", err)
		}
		...
		if multisig != "" {
			...
			if err != nil {
				...
				multisigAddr, _, _, err = client.GetFromFields(txFactory.Keybase(), multisig, clientCtx.GenerateOnly, clientCtx.Simulate)
				if err != nil {
					return fmt.Errorf("error getting account from keybase: %w", err)
				}
			}
