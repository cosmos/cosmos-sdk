...
func QueryTxsByEvents(clientCtx client.Context, page, limit int, query, orderBy string) (*sdk.SearchTxsResult, error) {
	...
	resTxs, err := node.TxSearch(clientCtx.GetCmdContextOr(context.Background()), query, false, &page, &limit, orderBy)
	if err != nil {
...
func QueryTx(clientCtx client.Context, hashHexStr string) (*sdk.TxResponse, error) {
	...
	resTx, err := node.Tx(clientCtx.GetCmdContextOr(context.Background()), hash, true)
	if err != nil {
		return nil, err
	}
...
func getBlocksForTxResults(clientCtx client.Context, resTxs []*coretypes.ResultTx) (map[int64]*coretypes.ResultBlock, error) {
	...
	for _, resTx := range resTxs {
		if _, ok := resBlocks[resTx.Height]; !ok {
			resBlock, err := node.Block(clientCtx.GetCmdContextOr(context.Background()), &resTx.Height)
			if err != nil {
				return nil, err
			}
