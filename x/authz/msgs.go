...
func (msg *MsgGrant) SetAuthorization(a Authorization) error {
	...
	if !ok {
		return errorstypes.ErrPackAny.Wrapf("can't proto marshal %T", m)
	}
...
func (msg MsgRevoke) ValidateBasic() error {
	...
	if msg.MsgTypeUrl == "" {
		return errorstypes.ErrInvalidRequest.Wrap("missing method name")
	}
...
func (msg MsgExec) GetMessages() ([]sdk.Msg, error) {
	...
	for i, msgAny := range msg.Msgs {
		...
		if !ok {
			return nil, errorstypes.ErrInvalidRequest.Wrapf("messages contains %T which is not a sdk.MsgRequest", msgAny)
		}
		msgs[i] = msg
	}
...
func (msg MsgExec) ValidateBasic() error {
	...
	if len(msg.Msgs) == 0 {
		return errorstypes.ErrInvalidRequest.Wrapf("messages cannot be empty")
	}
