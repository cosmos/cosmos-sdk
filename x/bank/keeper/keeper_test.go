...
func (suite *IntegrationTestSuite) TestSupply_SendCoins() {
	...
	bal, _, _ := keeper.GetAllBalances(ctx, baseAcc.GetAddress(), nil)
	suite.Require().Equal(initCoins, bal)
...
func (suite *IntegrationTestSuite) TestSendCoinsNewAccount() {
	...
	acc1Balances, _, err := app.BankKeeper.GetAllBalances(ctx, addr1, nil)
	suite.Require().NoError(err)
	...
	acc2Balances, _, err := app.BankKeeper.GetAllBalances(ctx, addr2, nil)
	suite.Require().NoError(err)
	suite.Require().Empty(acc2Balances)

	sendAmt := sdk.NewCoins(newFooCoin(50), newBarCoin(25))
	suite.Require().NoError(app.BankKeeper.SendCoins(ctx, addr1, addr2, sendAmt))

	acc2Balances, _, _ = app.BankKeeper.GetAllBalances(ctx, addr2, nil)
