...
func (suite *KeeperTestSuite) TestMigrator_Migrate3to4() {
	...
	for _, def := range []bool{true, false} {
		...
		suite.T().Run(fmt.Sprintf("default %t send enabled info moved to store", def), func(t *testing.T) {
			...
			require.Len(newParams.SendEnabled, 0) //nolint:staticcheck // this is a test for the migration, so it needs to use the deprecated method
			require.Equal(def, newParams.DefaultSendEnabled)

			for _, se := range params.SendEnabled { //nolint:staticcheck // this is a test for the migration, so it needs to use the deprecated method
				actual := bankKeeper.IsSendEnabledDenom(ctx, se.Denom)
				require.Equal(se.Enabled, actual, se.Denom)
			}
		})
...
func (suite *KeeperTestSuite) TestSetParams() {
	...
	params.SendEnabled = []*banktypes.SendEnabled{ //nolint:staticcheck // SendEnabled is still a parameter, so we need to use it
	...
	suite.Run("stored params are as expected", func() {
		...
		require.Len(actual.SendEnabled, 0, "SendEnabled") //nolint:staticcheck // SendEnabled is still a parameter, so we need to use it
	})
