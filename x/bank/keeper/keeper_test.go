...
func (suite *KeeperTestSuite) TestSupply_SendCoins() {
	...
	require.Panics(func() {
		_ = keeper.SendCoinsFromModuleToModule(ctx, "", holderAcc.GetName(), initCoins) //nolint:errcheck // we're looking for a panic, not an error
	})
	...
	require.Panics(func() {
		_ = keeper.SendCoinsFromModuleToModule(ctx, authtypes.Burner, "", initCoins) //nolint:errcheck // we're looking for a panic, not an error
	})

	authKeeper.EXPECT().GetModuleAddress("").Return(nil)
	require.Panics(func() {
		_ = keeper.SendCoinsFromModuleToAccount(ctx, "", baseAcc.GetAddress(), initCoins) ///nolint:errcheck // we're looking for a panic, not an error
	})
...
func (suite *KeeperTestSuite) TestSupply_MintCoins() {
	...
	authKeeper.EXPECT().GetModuleAccount(ctx, "").Return(nil)
	require.Panics(func() { _ = keeper.MintCoins(ctx, "", initCoins) }, "no module account") //nolint:errcheck // we're looking for a panic, not an error

	suite.mockMintCoins(burnerAcc)
	require.Panics(func() { _ = keeper.MintCoins(ctx, authtypes.Burner, initCoins) }, "invalid permission") //nolint:errcheck // we're looking for a panic, not an error
	...
	require.Panics(func() { _ = keeper.MintCoins(ctx, randomPerm, initCoins) }) //nolint:errcheck // we're looking for a panic, not an error
...
func (suite *KeeperTestSuite) TestSupply_BurnCoins() {
	...
	authKeeper.EXPECT().GetModuleAccount(ctx, "").Return(nil)
	require.Panics(func() { _ = keeper.BurnCoins(ctx, "", initCoins) }, "no module account") //nolint:errcheck // we're looking for a panic, not an error

	authKeeper.EXPECT().GetModuleAccount(ctx, minterAcc.Name).Return(nil)
	require.Panics(func() { _ = keeper.BurnCoins(ctx, authtypes.Minter, initCoins) }, "invalid permission") //nolint:errcheck // we're looking for a panic, not an error

	authKeeper.EXPECT().GetModuleAccount(ctx, randomPerm).Return(nil)
	require.Panics(func() { _ = keeper.BurnCoins(ctx, randomPerm, supplyAfterInflation) }, "random permission") //nolint:errcheck // we're looking for a panic, not an error
