...
func (suite *KeeperTestSuite) mockSendCoins(ctx sdk.Context, sender authtypes.IAccount, receiver sdk.AccAddress) {
...
func (suite *KeeperTestSuite) mockInputOutputCoins(inputs []authtypes.IAccount, outputs []sdk.AccAddress) {
...
func (suite *KeeperTestSuite) mockValidateBalance(acc authtypes.IAccount) {
	suite.authKeeper.EXPECT().GetAccount(suite.ctx, acc.GetAddress()).Return(acc)
}

func (suite *KeeperTestSuite) mockSpendableCoins(ctx sdk.Context, acc authtypes.IAccount) {
	suite.authKeeper.EXPECT().GetAccount(ctx, acc.GetAddress()).Return(acc)
}

func (suite *KeeperTestSuite) mockDelegateCoins(ctx sdk.Context, acc authtypes.IAccount, mAcc authtypes.IAccount) {
...
func (suite *KeeperTestSuite) mockUnDelegateCoins(ctx sdk.Context, acc authtypes.IAccount, mAcc authtypes.IAccount) {
	vacc, ok := acc.(banktypes.VestingAccount)
	if ok {
		suite.authKeeper.EXPECT().SetAccount(ctx, vacc)
	}
...
func (suite *KeeperTestSuite) TestInputOutputNewAccount() {
	...
	suite.mockInputOutputCoins([]authtypes.IAccount{authtypes.NewBaseAccountWithAddress(accAddrs[0])}, []sdk.AccAddress{accAddrs[1]})
...
func (suite *KeeperTestSuite) TestInputOutputCoins() {
	...
	suite.mockInputOutputCoins([]authtypes.IAccount{acc0}, accAddrs[1:3])
	require.NoError(suite.bankKeeper.InputOutputCoins(ctx, inputs, outputs))
...
func (suite *KeeperTestSuite) TestMsgMultiSendEvents() {
	...
	suite.mockInputOutputCoins([]authtypes.IAccount{acc0}, accAddrs[2:4])
	require.NoError(suite.bankKeeper.InputOutputCoins(ctx, inputs, outputs))
