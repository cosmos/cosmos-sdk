...
func TestBeginBlockNoOp(t *testing.T) {
	...
	ctx := testCtx.Ctx.WithBlockHeader(cmtproto.Header{Time: time.Now()}).WithBlockHeight(0)

	bankKeeper := distrtestutil.NewMockBankKeeper(ctrl)
	stakingKeeper := distrtestutil.NewMockStakingKeeper(ctrl)
	accountKeeper := distrtestutil.NewMockAccountKeeper(ctrl)

	accountKeeper.EXPECT().GetModuleAddress(disttypes.ModuleName).Return(distrAcc.GetAddress())
	...
	err := distrKeeper.BeginBlocker(ctx)
...
func TestBeginBlockToManyValidators(t *testing.T) {
	...
	testCtx := testutil.DefaultContextWithDB(t, key, storetypes.NewTransientStoreKey("transient_test"))
	encCfg := moduletestutil.MakeTestEncodingConfig(distribution.AppModuleBasic{})
	ctx := testCtx.Ctx.WithBlockHeader(cmtproto.Header{Time: time.Now()}).WithBlockHeight(1)
