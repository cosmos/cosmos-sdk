...
func TestAllocateTokensToValidatorWithCommission(t *testing.T) {
	...
	// allocate tokens
	tokens := sdk.DecCoins{
		{sdk.BondDenom, sdk.NewDec(10)},
	}
	k.AllocateTokensToValidator(ctx, val, tokens)

	// check commission
	expected := sdk.DecCoins{
		{sdk.BondDenom, sdk.NewDec(5)},
	}
...
func TestAllocateTokensToManyValidators(t *testing.T) {
	...
	require.Equal(t, sdk.DecCoins{{sdk.BondDenom, sdk.NewDecWithPrec(465, 1)}}, k.GetValidatorOutstandingRewards(ctx, valOpAddr1))
	require.Equal(t, sdk.DecCoins{{sdk.BondDenom, sdk.NewDecWithPrec(515, 1)}}, k.GetValidatorOutstandingRewards(ctx, valOpAddr2))
	// 2 community pool coins
	require.Equal(t, sdk.DecCoins{{sdk.BondDenom, sdk.NewDec(2)}}, k.GetFeePool(ctx).CommunityPool)
	// 50% commission for first proposer, (0.5 * 93%) * 100 / 2 = 23.25
	require.Equal(t, sdk.DecCoins{{sdk.BondDenom, sdk.NewDecWithPrec(2325, 2)}}, k.GetValidatorAccumulatedCommission(ctx, valOpAddr1))
	// zero commission for second proposer
	require.True(t, k.GetValidatorAccumulatedCommission(ctx, valOpAddr2).IsZero())
	// just staking.proportional for first proposer less commission = (0.5 * 93%) * 100 / 2 = 23.25
	require.Equal(t, sdk.DecCoins{{sdk.BondDenom, sdk.NewDecWithPrec(2325, 2)}}, k.GetValidatorCurrentRewards(ctx, valOpAddr1).Rewards)
	// proposer reward + staking.proportional for second proposer = (5 % + 0.5 * (93%)) * 100 = 51.5
	require.Equal(t, sdk.DecCoins{{sdk.BondDenom, sdk.NewDecWithPrec(515, 1)}}, k.GetValidatorCurrentRewards(ctx, valOpAddr2).Rewards)
}
...
func TestAllocateTokensTruncation(t *testing.T) {
	...
	msg := staking.NewMsgCreateValidator(valOpAddr1, valConsPk1,
		sdk.NewCoin(sdk.BondDenom, sdk.NewInt(110)), staking.Description{}, commission, sdk.OneInt())
	require.True(t, sh(ctx, msg).IsOK())
	...
	// allocate tokens as if both had voted and second was proposer
	fees := sdk.Coins{
		sdk.NewCoin(sdk.BondDenom, sdk.NewInt(634195840)),
	}
	fck.SetCollectedFees(fees)
	votes := []abci.VoteInfo{
		{
