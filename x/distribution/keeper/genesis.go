...
func (k Keeper) InitGenesis(ctx sdk.Context, data types.GenesisState) {
	...
	for _, dwi := range data.DelegatorWithdrawInfos {
		delegatorAddress, err := k.authKeeper.GetAddressCodec().StringToBytes(dwi.DelegatorAddress)
		if err != nil {
			panic(err)
		}
		withdrawAddress, err := k.authKeeper.GetAddressCodec().StringToBytes(dwi.WithdrawAddress)
		if err != nil {
			panic(err)
		}
	...
	for _, del := range data.DelegatorStartingInfos {
		...
		delegatorAddress, err := k.authKeeper.GetAddressCodec().StringToBytes(del.DelegatorAddress)
		if err != nil {
			panic(err)
		}
