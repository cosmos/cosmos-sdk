...
// GetDelegatorWithdrawAddr gets the delegator withdraw address, defaulting to the delegator address
...
func (k Keeper) DeleteDelegatorWithdrawAddr(ctx context.Context, delAddr, _ sdk.AccAddress) error {
	store := k.storeService.OpenKVStore(ctx)
...
// GetValidatorHistoricalReferenceCount gets the historical reference count (used for testcases)
