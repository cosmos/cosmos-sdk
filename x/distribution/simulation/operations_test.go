...
func (suite *SimTestSuite) TestSimulateMsgSetWithdrawAddress() {
	...
	suite.Require().Equal(types.TypeMsgSetWithdrawAddress, sdk.MsgTypeURL(msg))
...
func (suite *SimTestSuite) TestSimulateMsgWithdrawDelegatorReward() {
	...
	suite.Require().Equal(types.TypeMsgWithdrawDelegatorReward, sdk.MsgTypeURL(msg))
	suite.Require().Equal(types.ModuleName, msg.Route())
	suite.Require().Len(futureOperations, 0)
}
...
func (suite *SimTestSuite) testSimulateMsgWithdrawValidatorCommission(tokenName string) {
	...
	if !operationMsg.OK {
		...
		suite.Require().Equal(types.TypeMsgWithdrawValidatorCommission, sdk.MsgTypeURL(msg))
		suite.Require().Equal(types.ModuleName, msg.Route())
		suite.Require().Len(futureOperations, 0)
	}
}
...
func (suite *SimTestSuite) TestSimulateMsgFundCommunityPool() {
	...
	suite.Require().Equal(types.TypeMsgFundCommunityPool, sdk.MsgTypeURL(msg))
	suite.Require().Equal(types.ModuleName, msg.Route())
	suite.Require().Len(futureOperations, 0)
}
