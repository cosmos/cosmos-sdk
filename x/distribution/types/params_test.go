...
func TestParams_ValidateBasic(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			params: types.Params{
				...
				NakamotoBonus: types.NakamotoBonus{
					Enabled:               true,
					PeriodEpochIdentifier: types.DefaultNakamotoBonusPeriodEpochIdentifier,
		...
		{
			name: "nakamoto bonus period empty",
			params: types.Params{
				CommunityTax:        toDec("0.1"),
				WithdrawAddrEnabled: true,
				NakamotoBonus: types.NakamotoBonus{
			...
			err: fmt.Errorf("nakamoto bonus period epoch identifier must not be empty"),
		},
	}
