...
func (msg MsgGrantAllowance) ValidateBasic() error {
	if _, err := sdk.AccAddressFromBech32(msg.Granter); err != nil {
		return errorstypes.ErrInvalidAddress.Wrapf("invalid granter address: %s", err)
	}
	if _, err := sdk.AccAddressFromBech32(msg.Grantee); err != nil {
		return errorstypes.ErrInvalidAddress.Wrapf("invalid grantee address: %s", err)
	}
...
func (msg MsgRevokeAllowance) ValidateBasic() error {
	...
	if msg.Grantee == msg.Granter {
		return errorstypes.ErrInvalidAddress.Wrap("addresses must be different")
	}
