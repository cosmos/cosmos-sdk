...
func NewTxCmd(legacyPropCmds []*cobra.Command) *cobra.Command {
	...
	govTxCmd.AddCommand(
		...
		NewCmdDraftProposal(),
		NewCmdSubmitProposalWithValidation(),
...
// NewCmdSubmitProposalWithValidation implements submitting a proposal transaction command with validation as it was in sdk v0.45.
func NewCmdSubmitProposalWithValidation() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "submit-proposal-validate [path/to/proposal.json]",
...
Example:
$ %s tx gov submit-proposal-validate path/to/proposal.json
