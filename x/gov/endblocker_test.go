...
func TestParamchangeProposalPassedEndblocker(t *testing.T) {
	input := getMockApp(t, 1, GenesisState{}, nil)
	SortAddresses(input.addrs)
	...
	dparam := input.keeper.GetDepositParams(ctx)

	proposal, err := input.keeper.SubmitProposal(ctx,
		params.NewParameterChangeProposal("title", "description",
			[]params.ParamChange{params.NewParamChange("testgov", string(ParamStoreKeyDepositParams), `{"max_deposit_period": "1"}`)},
		),
	)
	require.NoError(t, err)
	...
	newparam := input.keeper.GetDepositParams(ctx)
	require.Equal(t, time.Duration(1), newparam.MaxDepositPeriod)
	require.Equal(t, dparam.MinDeposit, newparam.MinDeposit)
}
