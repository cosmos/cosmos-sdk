...
func (keeper Keeper) SetDeposit(ctx context.Context, deposit v1.Deposit) error {
	depositor, err := keeper.authKeeper.GetAddressCodec().StringToBytes(deposit.Depositor)
	if err != nil {
		return err
	}
...
func (keeper Keeper) ChargeDeposit(ctx context.Context, proposalID uint64, destAddress, proposalCancelRate string) error {
	...
	for _, deposit := range deposits {
		depositerAddress, err := keeper.authKeeper.GetAddressCodec().StringToBytes(deposit.Depositor)
		if err != nil {
			return err
		}
	...
	if !cancellationCharges.IsZero() {
		...
		default:
			destAccAddress, err := keeper.authKeeper.GetAddressCodec().StringToBytes(destAddress)
			if err != nil {
				return err
			}
			err = keeper.bankKeeper.SendCoinsFromModuleToAccount(
