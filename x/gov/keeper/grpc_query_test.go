	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				msgs := createTestProposalMsgs(ctx, app.GovKeeper)
				submittedProposal, err := app.GovKeeper.SubmitProposal(ctx, msgs)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				for i := 0; i < 5; i++ {
					msgs := createTestProposalMsgs(ctx, app.GovKeeper)
					proposal, err := app.GovKeeper.SubmitProposal(ctx, msgs)
