	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.Require().NoError(suite.govKeeper.AddVote(ctx, proposal.Id, addrs[0], v1.NewNonSplitVoteOption(v1.OptionYes), ""))
				suite.Require().NoError(suite.govKeeper.AddVote(ctx, proposal.Id, addrs[1], v1.NewNonSplitVoteOption(v1.OptionYes), ""))
				suite.Require().NoError(suite.govKeeper.AddVote(ctx, proposal.Id, addrs[2], v1.NewNonSplitVoteOption(v1.OptionYes), ""))
		...
		},
		{
			...
			func() {
				...
				suite.govKeeper.SetProposal(ctx, proposal)
				proposal, _ = suite.govKeeper.GetProposal(ctx, proposal.Id)
