	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				submittedProposal, err := suite.govKeeper.SubmitProposal(ctx, []sdk.Msg{msgContent}, "", "title", "summary", addrs[0], false, nil)
				suite.Require().NoError(err)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				msgContent, err := v1.NewLegacyContent(testProposal, govAcct.String())
				suite.Require().NoError(err)
				submittedProposal, err := suite.govKeeper.SubmitProposal(ctx, []sdk.Msg{msgContent}, "", "title", "summary", addrs[0], true, nil)
				suite.Require().NoError(err)
				suite.Require().NotEmpty(submittedProposal)
		...
		},
		{
			...
			func() {
				...
				for i := range 5 {
					...
					proposal, err := suite.govKeeper.SubmitProposal(ctx, testProposal, "", "title", "summary", addrs[0], false, nil)
...
func (suite *KeeperTestSuite) TestGRPCQueryDeposits() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				var err error
				proposal, err = suite.govKeeper.SubmitProposal(ctx, TestProposal, "", "title", "summary", addrs[0], true, nil)
				suite.Require().NoError(err)
