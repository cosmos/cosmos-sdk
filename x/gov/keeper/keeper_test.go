...
func TestIncrementProposalNumber(t *testing.T) {
	...
	tp := TestProposal
	_, err = govKeeper.SubmitProposal(ctx, tp, "", "test", "summary", addrBz)
	require.NoError(t, err)
	_, err = govKeeper.SubmitProposal(ctx, tp, "", "test", "summary", addrBz)
	require.NoError(t, err)
	_, err = govKeeper.SubmitProposal(ctx, tp, "", "test", "summary", addrBz)
	...
	_, err = govKeeper.SubmitProposal(ctx, tp, "", "test", "summary", addrBz)
	require.NoError(t, err)
	proposal6, err := govKeeper.SubmitProposal(ctx, tp, "", "test", "summary", addrBz)
	require.NoError(t, err)
...
func TestProposalQueues(t *testing.T) {
	...
	tp := TestProposal
	proposal, err := govKeeper.SubmitProposal(ctx, tp, "", "test", "summary", addrBz)
	require.NoError(t, err)
