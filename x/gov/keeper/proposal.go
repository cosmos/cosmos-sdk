...
func (keeper Keeper) SubmitProposal(ctx sdk.Context, messages []sdk.Msg, metadata string) (v1.Proposal, error) {
	...
	for _, msg := range messages {
		...
		if err := msg.ValidateBasic(); err != nil {
			return v1.Proposal{}, types.ErrInvalidProposalMsg.Wrap(err.Error())
		}
		...
		if !signers[0].Equals(keeper.GetGovernanceAccount(ctx).GetAddress()) {
			return v1.Proposal{}, types.ErrInvalidSigner.Wrap(signers[0].String())
		}

		// use the msg service router to see that there is a valid route for that message.
		handler := keeper.router.Handler(msg)
		if handler == nil {
			return v1.Proposal{}, types.ErrUnroutableProposalMsg.Wrap(sdk.MsgTypeURL(msg))
		}
		...
		if msg, ok := msg.(*v1.MsgExecLegacyContent); ok {
			...
			if _, err := handler(cacheCtx, msg); err != nil {
				return v1.Proposal{}, types.ErrNoProposalHandlerExists.Wrap(err.Error())
			}
		}
...
func (keeper Keeper) GetProposalID(ctx sdk.Context) (proposalID uint64, err error) {
	...
	if bz == nil {
		return 0, types.ErrInvalidGenesis.Wrap("initial proposal ID hasn't been set")
	}
