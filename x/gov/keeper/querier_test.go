...
func TestQueries(t *testing.T) {
	...
	proposal1, err := app.GovKeeper.SubmitProposal(ctx, []sdk.Msg{tp})
	require.NoError(t, err)
	...
	proposal2, err := app.GovKeeper.SubmitProposal(ctx, []sdk.Msg{tp})
	require.NoError(t, err)
	...
	proposal3, err := app.GovKeeper.SubmitProposal(ctx, []sdk.Msg{tp})
	require.NoError(t, err)
