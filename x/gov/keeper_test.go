...
func TestDeposits(t *testing.T) {
	...
	fourUatom := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, sdk.TokensFromTendermintPower(4)))
	fiveUatom := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, sdk.TokensFromTendermintPower(5)))
	...
	// Check first deposit
	err, votingStarted := keeper.AddDeposit(ctx, proposalID, addrs[0], fourUatom)
	require.Nil(t, err)
	require.False(t, votingStarted)
	deposit, found = keeper.GetDeposit(ctx, proposalID, addrs[0])
	require.True(t, found)
	require.Equal(t, fourUatom, deposit.Amount)
	require.Equal(t, addrs[0], deposit.Depositor)
	require.Equal(t, fourUatom, keeper.GetProposal(ctx, proposalID).GetTotalDeposit())
	require.Equal(t, addr0Initial.Sub(fourUatom), keeper.ck.GetCoins(ctx, addrs[0]))

	// Check a second deposit from same address
	err, votingStarted = keeper.AddDeposit(ctx, proposalID, addrs[0], fiveUatom)
	require.Nil(t, err)
	require.False(t, votingStarted)
	deposit, found = keeper.GetDeposit(ctx, proposalID, addrs[0])
	require.True(t, found)
	require.Equal(t, fourUatom.Add(fiveUatom), deposit.Amount)
	require.Equal(t, addrs[0], deposit.Depositor)
	require.Equal(t, fourUatom.Add(fiveUatom), keeper.GetProposal(ctx, proposalID).GetTotalDeposit())
	require.Equal(t, addr0Initial.Sub(fourUatom).Sub(fiveUatom), keeper.ck.GetCoins(ctx, addrs[0]))

	// Check third deposit from a new address
	err, votingStarted = keeper.AddDeposit(ctx, proposalID, addrs[1], fourUatom)
	require.Nil(t, err)
	...
	require.Equal(t, fourUatom, deposit.Amount)
	require.Equal(t, fourUatom.Add(fiveUatom).Add(fourUatom), keeper.GetProposal(ctx, proposalID).GetTotalDeposit())
	require.Equal(t, addr1Initial.Sub(fourUatom), keeper.ck.GetCoins(ctx, addrs[1]))
