...
func TestDeposits(t *testing.T) {
	...
	fourSteak := sdk.Coins{sdk.NewInt64Coin("stake", 4)}
	fiveSteak := sdk.Coins{sdk.NewInt64Coin("stake", 5)}

	addr0Initial := keeper.ck.GetCoins(ctx, addrs[0])
	addr1Initial := keeper.ck.GetCoins(ctx, addrs[1])

	// require.True(t, addr0Initial.IsEqual(sdk.Coins{sdk.NewInt64Coin("stake", 42)}))
	require.Equal(t, sdk.Coins{sdk.NewInt64Coin("stake", 42)}, addr0Initial)
