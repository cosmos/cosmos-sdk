...
func NewSingleValueIterator(rowID RowID, val []byte) Iterator {
	...
	return IteratorFunc(func(dest codec.ProtoMarshaler) (RowID, error) {
		if dest == nil {
			return nil, errors.ErrORMInvalidArgument.Wrap("destination object must not be nil")
		}
		if closed || val == nil {
...
func First(it Iterator, dest codec.ProtoMarshaler) (RowID, error) {
	if it == nil {
		return nil, errors.ErrORMInvalidArgument.Wrap("iterator must not be nil")
	}
	defer it.Close()
...
func Paginate(
	...
	for {
		...
		if !ok {
			return nil, errors.ErrORMInvalidArgument.Wrapf("%s should implement codec.ProtoMarshaler", elemType)
		}
...
func assertDest(dest ModelSlicePtr, destRef *reflect.Value, tmpSlice *reflect.Value) (reflect.Type, error) {
	if dest == nil {
		return nil, errors.ErrORMInvalidArgument.Wrap("destination must not be nil")
	}
	tp := reflect.ValueOf(dest)
	if tp.Kind() != reflect.Ptr {
		return nil, errors.ErrORMInvalidArgument.Wrap("destination must be a pointer to a slice")
	}
	if tp.Elem().Kind() != reflect.Slice {
		return nil, errors.ErrORMInvalidArgument.Wrap("destination must point to a slice")
	}
	...
	*destRef = tp.Elem()
	...
	if !destRef.CanSet() {
		return nil, errors.ErrORMInvalidArgument.Wrap("destination not assignable")
	}
	...
	if !elemType.Implements(protoMarshaler) &&
		...
		return nil, errors.ErrORMInvalidArgument.Wrapf("unsupported type :%s", elemType)
	}
