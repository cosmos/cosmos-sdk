...
func NewGroupKeeper(storeKey storetypes.StoreKey, paramSpace params.Subspace, router *app.MsgServiceRouter, cdc codec.Codec) Keeper {
	...
	groupTable, err := orm.NewAutoUInt64Table([2]byte{GroupTablePrefix}, GroupTableSeqPrefix, &GroupInfo{}, cdc)
	if err != nil {
		panic(err.Error())
	}
	k.groupByAdminIndex, err = orm.NewIndex(groupTable, GroupByAdminIndexPrefix, func(val interface{}) ([]interface{}, error) {
		addr, err := sdk.AccAddressFromBech32(val.(*GroupInfo).Admin)
		if err != nil {
			return nil, err
		}
	...
	// Group Member Table
	groupMemberTable, err := orm.NewPrimaryKeyTable([2]byte{GroupMemberTablePrefix}, &GroupMember{}, cdc)
	if err != nil {
		panic(err.Error())
	}
	...
	k.groupMemberByMemberIndex, err = orm.NewIndex(groupMemberTable, GroupMemberByMemberIndexPrefix, func(val interface{}) ([]interface{}, error) {
		memberAddr := val.(*GroupMember).Member.Address
	...
	groupAccountTable, err := orm.NewPrimaryKeyTable([2]byte{GroupAccountTablePrefix}, &GroupAccountInfo{}, cdc)
	if err != nil {
		panic(err.Error())
	}
	k.groupAccountByGroupIndex, err = orm.NewIndex(groupAccountTable, GroupAccountByGroupIndexPrefix, func(value interface{}) ([]interface{}, error) {
		return []interface{}{value.(*GroupAccountInfo).GroupId}, nil
	}, GroupAccountInfo{}.GroupId)
	if err != nil {
		panic(err.Error())
	}
	k.groupAccountByAdminIndex, err = orm.NewIndex(groupAccountTable, GroupAccountByAdminIndexPrefix, func(value interface{}) ([]interface{}, error) {
		admin := value.(*GroupAccountInfo).Admin
	...
	// Proposal Table
	proposalTable, err := orm.NewAutoUInt64Table([2]byte{ProposalTablePrefix}, ProposalTableSeqPrefix, &Proposal{}, cdc)
	if err != nil {
		panic(err.Error())
	}
	k.proposalByGroupAccountIndex, err = orm.NewIndex(proposalTable, ProposalByGroupAccountIndexPrefix, func(value interface{}) ([]interface{}, error) {
		account := value.(*Proposal).Address
	...
	k.proposalByProposerIndex, err = orm.NewIndex(proposalTable, ProposalByProposerIndexPrefix, func(value interface{}) ([]interface{}, error) {
		proposers := value.(*Proposal).Proposers
...
// func NewKeeper(storeKey storetypes.StoreKey, cdc codec.BinaryCodec, ak AccountKeeper, bk BankKeeper) Keeper {
// 	return Keeper{
// 		cdc:        cdc,
// 		storeKey:   storeKey,
// 		authKeeper: ak,
