...
func (suite *KeeperTestSuite) SetupTest() {
	...
	suite.cdc = app.Codec()
	suite.ctx = app.BaseApp.NewContext(isCheckTx, tmproto.Header{Height: int64(testClientHeight.EpochHeight), ChainID: testClientID, Time: now2})
...
func (suite KeeperTestSuite) TestConsensusStateHelpers() {
	...
	header := ibctmtypes.CreateTestHeader(
		testClientID, exported.NewHeight(0, testClientHeight.EpochHeight+5), testClientHeight, suite.header.Time.Add(time.Minute), suite.valSet, suite.valSet, []tmtypes.PrivValidator{suite.privVal},
	)
