...
// any merkle proof.
func QueryAllConnections(cliCtx context.Context, page, limit int) ([]types.ConnectionEnd, int64, error) {
...
func QueryConnection(
	cliCtx context.Context, connectionID string, prove bool,
...
func QueryAllClientConnectionPaths(cliCtx context.Context, page, limit int) ([]types.ConnectionPaths, int64, error) {
	params := types.NewQueryAllConnectionsParams(page, limit)
	bz, err := cliCtx.Codec.MarshalJSON(params)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to marshal query params: %w", err)
