...
func (k Keeper) ConnOpenInit(
	...
	connection := types.NewConnectionEnd(common.INIT, connectionID, clientID, counterparty, types.GetCompatibleVersions())
...
func (k Keeper) ConnOpenTry(
	...
	expectedConnection := types.NewConnectionEnd(common.INIT, counterparty.ConnectionID, counterparty.ClientID, expectedCounterparty, counterpartyVersions)
	...
	connection := types.NewConnectionEnd(common.UNINITIALIZED, connectionID, clientID, counterparty, []string{version})
	...
	if found && !(previousConnection.State == common.INIT &&
	...
	connection.State = common.TRYOPEN
	if err := k.addConnectionToClient(ctx, clientID, connectionID); err != nil {
...
func (k Keeper) ConnOpenAck(
	...
	expectedConnection := types.NewConnectionEnd(common.TRYOPEN, connection.Counterparty.ConnectionID, connection.Counterparty.ClientID, expectedCounterparty, []string{version})
...
func (k Keeper) ConnOpenConfirm(
	...
	if connection.State != common.TRYOPEN {
		return sdkerrors.Wrapf(
			types.ErrInvalidConnectionState,
	...
	prefix := k.GetCommitmentPrefix()
	expectedCounterparty := types.NewCounterparty(connection.ClientID, connectionID, commitmenttypes.NewMerklePrefix(prefix.Bytes()))
	expectedConnection := types.NewConnectionEnd(common.OPEN, connection.Counterparty.ConnectionID, connection.Counterparty.ClientID, expectedCounterparty, connection.Versions)
