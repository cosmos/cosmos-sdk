...
func (suite *KeeperTestSuite) TestSetAndGetClientConnectionPaths() {
	...
	suite.chainA.App.IBCKeeper.ConnectionKeeper.SetClientConnectionPaths(suite.chainA.GetContext(), clientA, types.GetCompatibleVersionStrings())
	paths, existed := suite.chainA.App.IBCKeeper.ConnectionKeeper.GetClientConnectionPaths(suite.chainA.GetContext(), clientA)
	suite.True(existed)
	suite.EqualValues(types.GetCompatibleVersionStrings(), paths)
}
...
func (suite KeeperTestSuite) TestGetAllConnections() {
	...
	conn1 := types.NewConnectionEnd(types.OPEN, connA0.ID, clientA, counterpartyB0, types.GetCompatibleVersionStrings()) // A0 - B0
	conn2 := types.NewConnectionEnd(types.OPEN, connA1.ID, clientA, counterpartyB1, types.GetCompatibleVersionStrings()) // A1 - B1
