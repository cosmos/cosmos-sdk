...
func (suite *KeeperTestSuite) TestTimeoutPacket() {
	...
	testCases := []testCase{
		{"success: ORDERED", func() {
			...
			packet = types.NewPacket(validPacketData, 1, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, clienttypes.NewHeight(0, uint64(suite.chainB.GetContext().BlockHeight())), uint64(suite.chainB.GetContext().BlockTime().UnixNano()))
		...
		{"success: UNORDERED", func() {
			...
			packet = types.NewPacket(validPacketData, 1, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, clienttypes.NewHeight(0, uint64(suite.chainB.GetContext().BlockHeight())), disabledTimeoutTimestamp)
			suite.coordinator.SendPacket(suite.chainA, suite.chainB, packet, clientB)
			// need to update chainA's client representing chainB to prove missing ack
			suite.coordinator.UpdateClient(suite.chainA, suite.chainB, clientA, clientexported.Tendermint)
		}, true},
