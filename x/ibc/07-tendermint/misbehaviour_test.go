...
func (suite *TendermintTestSuite) TestCheckMisbehaviour() {
	...
	testCases := []struct {
		...
		{
			"valid misbehaviour at height in future epoch from last consensusState",
			ibctmtypes.NewClientState(chainID, lite.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, suite.header, commitmenttypes.GetSDKSpecs()),
			ibctmtypes.ConsensusState{Timestamp: suite.now, Root: commitmenttypes.NewMerkleRoot(tmhash.Sum([]byte("app_hash"))), ValidatorSet: bothValSet},
			ibctmtypes.Evidence{
				Header1:  ibctmtypes.CreateTestHeader(chainID, clientexported.NewHeight(3, 1), suite.now, bothValSet, bothSigners),
				Header2:  ibctmtypes.CreateTestHeader(chainID, clientexported.NewHeight(3, 1), suite.now.Add(time.Minute), bothValSet, bothSigners),
				ChainID:  chainID,
				ClientID: chainID,
			},
			simapp.DefaultConsensusParams,
		...
		{
			"invalid misbehaviour, heights are at two different epochs",
			ibctmtypes.NewClientState(chainID, lite.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, suite.header, commitmenttypes.GetSDKSpecs()),
			ibctmtypes.ConsensusState{Timestamp: suite.now, Root: commitmenttypes.NewMerkleRoot(tmhash.Sum([]byte("app_hash"))), ValidatorSet: bothValSet},
			ibctmtypes.Evidence{
				Header1:  ibctmtypes.CreateTestHeader(chainID, height, suite.now, bothValSet, bothSigners),
				Header2:  ibctmtypes.CreateTestHeader(chainID, clientexported.NewHeight(3, height.EpochHeight), suite.now.Add(time.Minute), bothValSet, bothSigners),
				ChainID:  chainID,
				ClientID: chainID,
			},
			simapp.DefaultConsensusParams,
