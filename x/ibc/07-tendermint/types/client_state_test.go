...
func (suite *TendermintTestSuite) TestVerifyClientConsensusState() {
	testCases := []struct {
		...
		},
		{
			...
			clientState: &types.ClientState{LatestHeight: height, FrozenHeight: clientexported.NewHeight(0, height.EpochHeight-1)},
			consensusState: types.ConsensusState{
				Root: commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()),
			},
			prefix:  commitmenttypes.NewMerklePrefix([]byte("ibc")),
	...
	testCases := []struct {
		...
		},
		{
			"latest client height < height", func() {
				proofHeight = clientexported.NewHeight(0, clientState.LatestHeight.EpochHeight+1)
			}, false,
		},
		{
			"client is frozen", func() {
				clientState.FrozenHeight = clientexported.NewHeight(0, 1)
			}, false,
		},
		{
