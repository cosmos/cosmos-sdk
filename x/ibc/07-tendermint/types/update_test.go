...
func (suite *TendermintTestSuite) TestCheckHeaderAndUpdateState() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.AppHash), height, suite.valsHash)
		...
		},
		{
			...
			setup: func() {
				clientState = types.NewClientState(chainID, types.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, height, commitmenttypes.GetSDKSpecs())
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.AppHash), height, bothValSet.Hash())
		...
		},
		{
			...
			setup: func() {
				clientState = types.NewClientState(chainID, types.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, height, commitmenttypes.GetSDKSpecs())
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.AppHash), height-3, suite.valsHash)
	...
	for i, tc := range testCases {
		...
		expectedConsensus := &types.ConsensusState{
			...
			Root:               commitmenttypes.NewMerkleRoot(newHeader.Header.AppHash),
