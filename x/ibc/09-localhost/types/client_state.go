...
func (cs ClientState) VerifyClientConsensusState(
	...
	if len(data) == 0 {
		return cs, sdkerrors.Wrapf(clienttypes.ErrFailedClientConsensusStateVerification, "not found for path %s", path)
	}
...
func (cs ClientState) VerifyConnectionState(
	...
	if bz == nil {
		return cs, sdkerrors.Wrapf(clienttypes.ErrFailedConnectionStateVerification, "not found for path %s", path)
	}
...
func (cs ClientState) VerifyChannelState(
	...
	bz := cs.store.Get([]byte(path.String()))
	if bz == nil {
		return cs, sdkerrors.Wrapf(clienttypes.ErrFailedChannelStateVerification, "not found for path %s", path)
	}
...
func (cs ClientState) VerifyPacketCommitment(
	...
	data := cs.store.Get([]byte(path.String()))
	if len(data) == 0 {
		return cs, sdkerrors.Wrapf(clienttypes.ErrFailedPacketCommitmentVerification, "not found for path %s", path)
	}
...
func (cs ClientState) VerifyPacketAcknowledgement(
	...
	data := cs.store.Get([]byte(path.String()))
	if len(data) == 0 {
		return cs, sdkerrors.Wrapf(clienttypes.ErrFailedPacketAckVerification, "not found for path %s", path)
	}
...
func (cs ClientState) VerifyPacketAcknowledgementAbsence(
	...
	data := cs.store.Get([]byte(path.String()))
	if data != nil {
		return cs, sdkerrors.Wrap(clienttypes.ErrFailedPacketAckAbsenceVerification, "expected no ack absence")
	}
...
func (cs ClientState) VerifyNextSequenceRecv(
	...
	data := cs.store.Get([]byte(path.String()))
	if len(data) == 0 {
		return cs, sdkerrors.Wrapf(clienttypes.ErrFailedNextSeqRecvVerification, "not found for path %s", path)
	}
