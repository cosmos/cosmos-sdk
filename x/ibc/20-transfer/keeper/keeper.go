...
func (k Keeper) Logger(ctx sdk.Context) log.Logger {
	return ctx.Logger().With("module", fmt.Sprintf("x/%s/%s", common.ModuleName, types.ModuleName))
}
...
func (k Keeper) PacketExecuted(ctx sdk.Context, packet channelexported.PacketI, acknowledgement []byte) error {
	chanCap, ok := k.scopedKeeper.GetCapability(ctx, common.ChannelCapabilityPath(packet.GetDestPort(), packet.GetDestChannel()))
	if !ok {
...
func (k Keeper) ChanCloseInit(ctx sdk.Context, portID, channelID string) error {
	capName := common.ChannelCapabilityPath(portID, channelID)
...
func (k Keeper) BindPort(ctx sdk.Context, portID string) error {
	...
	return k.ClaimCapability(ctx, cap, common.PortPath(portID))
}
