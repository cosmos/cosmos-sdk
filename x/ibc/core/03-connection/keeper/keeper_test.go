...
func (suite KeeperTestSuite) TestGetAllConnections() {
	...
	conn1 := types.NewConnectionEnd(types.OPEN, clientA, counterpartyB0, types.ExportedToProto(types.GetCompatibleVersions())) // A0 - B0
	conn2 := types.NewConnectionEnd(types.OPEN, clientA, counterpartyB1, types.ExportedToProto(types.GetCompatibleVersions())) // A1 - B1
