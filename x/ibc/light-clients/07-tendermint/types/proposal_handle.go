...
func (cs ClientState) CheckProposedHeaderAndUpdateState(
	...
	case cs.IsFrozen():
		...
		if cs.IsExpired(consensusState.Timestamp, ctx.BlockTime()) {
			return cs.unexpireClient(ctx, consensusState, tmHeader, ctx.BlockTime())
		}
...
func (cs ClientState) unexpireClient(
	ctx sdk.Context, consensusState *ConsensusState, header *Header, currentTimestamp time.Time,
) (exported.ClientState, exported.ConsensusState, error) {
	...
	newClientState, consensusState := update(ctx, &cs, header)
