...
func (s *TestSuite) TestKeeper() {
	...
	capability := s.keeper.GetCapability(s.ctx, granteeAddr, granterAddr, bank.MsgSend{})
	...
	capability = s.keeper.GetCapability(s.ctx, granteeAddr, granterAddr, bank.MsgMultiSend{})
	s.Require().Nil(capability)

	s.T().Log("Verify fetching capability with wrong grantee fails")
	capability = s.keeper.GetCapability(s.ctx, recepientAddr, granterAddr, bank.MsgMultiSend{})
	s.Require().Nil(capability)

	s.T().Log("Verify revoke fails with wrong information")
	s.keeper.Revoke(s.ctx, recepientAddr, granterAddr, bank.MsgSend{})
	capability = s.keeper.GetCapability(s.ctx, recepientAddr, granterAddr, bank.MsgSend{})
	s.Require().Nil(capability)
