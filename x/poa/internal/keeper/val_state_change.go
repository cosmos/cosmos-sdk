...
func (k Keeper) ApplyAndReturnValidatorSetUpdates(ctx sdk.Context) (updates []abci.ValidatorUpdate) {
	...
	for count := 0; iterator.Valid() && count < int(maxValidators); iterator.Next() {
		...
		case validator.IsUnbonded():
			...
			amtFromNotBondedToBonded = amtFromNotBondedToBonded.Add(validator.GetWeight())
	...
	for _, valAddrBytes := range noLongerBonded {
		...
		amtFromBondedToNotBonded = amtFromBondedToNotBonded.Add(validator.GetWeight())
...
// begins the state of unbonding for a bonded validator
...
// switches from the state of Unbonding back to Bonding
...
// switches a Unbonded validator to Bonded
