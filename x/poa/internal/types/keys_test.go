...
func TestGetValidatorPowerRank(t *testing.T) {
	...
	emptyDesc := staking.Description{}
	val1 := NewValidator(valAddr1, keysPK1, emptyDesc)
	val1.Weight = sdk.ZeroInt()
	val2, val3, val4 := val1, val1, val1
	val2.Weight = sdk.TokensFromConsensusPower(1)
	val3.Weight = sdk.TokensFromConsensusPower(10)
	x := new(big.Int).Exp(big.NewInt(2), big.NewInt(40), big.NewInt(0))
	val4.Weight = sdk.TokensFromConsensusPower(x.Int64())
...
func TestGetREDByValDstIndexKey(t *testing.T) {
	...
	for i, tt := range tests {
		got := hex.EncodeToString(stakingTypes.GetREDByValDstIndexKey(tt.delAddr, tt.valSrcAddr, tt.valDstAddr))
...
func TestGetREDByValSrcIndexKey(t *testing.T) {
	...
	for i, tt := range tests {
		got := hex.EncodeToString(stakingTypes.GetREDByValSrcIndexKey(tt.delAddr, tt.valSrcAddr, tt.valDstAddr))
