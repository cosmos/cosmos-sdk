...
// GetVoteArray returns array of missed votes for a particular validator.
// For new validator or slashed validator nil is returned.
...
// SetVoteArray updates vote array for a particular validator.
func (k Keeper) SetVoteArray(ctx sdk.Context, address sdk.ConsAddress, votes *types.VoteArray) {
	store := ctx.KVStore(k.storeKey)
	store.Set(types.GetValidatorMissedBlockBitArrayKey(address), votes.Bytes())
}

// ClearVoteArray delete vote array for a particular validator.
func (k Keeper) ClearVoteArray(ctx sdk.Context, address sdk.ConsAddress) {
	store := ctx.KVStore(k.storeKey)
	store.Delete(types.GetValidatorMissedBlockBitArrayKey(address))
}
