...
func (k Keeper) HandleDoubleSign(ctx sdk.Context, addr crypto.Address, infractionHeight int64, timestamp time.Time, power int64) {
	...
	if age > k.MaxEvidenceAge(ctx) {
		logger.Info(fmt.Sprintf("Ignored double sign from %s at height %d, age of %d past max age of %d",
			sdk.AccAddress(pubkey.Address()), infractionHeight, age, k.MaxEvidenceAge(ctx)))
		return
	}
	...
	if signInfo.Tombstoned {
		logger.Info(fmt.Sprintf("Ignored double sign from %s at height %d, validator already tombstoned", sdk.AccAddress(pubkey.Address()), infractionHeight))
		return
	}

	// double sign confirmed
	logger.Info(fmt.Sprintf("Confirmed double sign from %s at height %d, age of %d", sdk.AccAddress(pubkey.Address()), infractionHeight, age))
...
func (k Keeper) HandleValidatorSignature(ctx sdk.Context, addr crypto.Address, power int64, signed bool) {
	...
	if missed {
		logger.Info(fmt.Sprintf("Absent validator %s (%s) at height %d, %d missed, threshold %d", sdk.AccAddress(addr), pubkey, height, signInfo.MissedBlocksCounter, k.MinSignedPerWindow(ctx)))
	}
	...
	if height > minHeight && signInfo.MissedBlocksCounter > maxMissed {
		...
		if validator != nil && !validator.IsJailed() {
			...
			logger.Info(fmt.Sprintf("Validator %s past min height of %d and below signed blocks threshold of %d",
				sdk.AccAddress(pubkey.Address()), minHeight, k.MinSignedPerWindow(ctx)))
