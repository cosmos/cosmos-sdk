...
func BeginBlocker(ctx sdk.Context, req abci.RequestBeginBlock, sk Keeper) sdk.Tags {
	...
	for _, voteInfo := range req.LastCommitInfo.GetVotes() {
		sk.handleValidatorSignature(ctx, voteInfo.Validator.Address, uint64(voteInfo.Validator.Power), voteInfo.SignedLastBlock)
	}
	...
	for _, evidence := range req.ByzantineValidators {
		...
		case tmtypes.ABCIEvidenceTypeDuplicateVote:
			sk.handleDoubleSign(ctx, evidence.Validator.Address, uint64(evidence.Height), evidence.Time, uint64(evidence.Validator.Power))
		default:
