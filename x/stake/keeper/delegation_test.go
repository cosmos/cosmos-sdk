...
func TestDelegation(t *testing.T) {
	...
	bond1to1.Shares = sdk.NewDec(99, 0)
	keeper.SetDelegation(ctx, bond1to1)
	resBond, found = keeper.GetDelegation(ctx, addrDels[0], addrVals[0])
	require.True(t, found)
	require.True(t, bond1to1.Equal(resBond))
	...
	bond1to2 := types.Delegation{addrDels[0], addrVals[1], sdk.NewDec(9, 0), 0}
	bond1to3 := types.Delegation{addrDels[0], addrVals[2], sdk.NewDec(9, 0), 1}
	bond2to1 := types.Delegation{addrDels[1], addrVals[0], sdk.NewDec(9, 0), 2}
	bond2to2 := types.Delegation{addrDels[1], addrVals[1], sdk.NewDec(9, 0), 3}
	bond2to3 := types.Delegation{addrDels[1], addrVals[2], sdk.NewDec(9, 0), 4}
...
func TestUnbondDelegation(t *testing.T) {
	...
	var err error
	var amount sdk.Dec
	amount, err = keeper.unbond(ctx, addrDels[0], addrVals[0], sdk.NewDec(6, 0))
...
func TestGetRedelegationsFromValidator(t *testing.T) {
	...
	rd := types.Redelegation{
		...
		CreationHeight:   0,
		MinTime:          0,
		SharesSrc:        sdk.NewDec(5, 0),
		SharesDst:        sdk.NewDec(5, 0),
	}
...
func TestRedelegation(t *testing.T) {
	...
	// modify a records, save, and retrieve
	rd.SharesSrc = sdk.NewDec(21, 0)
	rd.SharesDst = sdk.NewDec(21, 0)
	keeper.SetRedelegation(ctx, rd)
