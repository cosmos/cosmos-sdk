...
import (
	...
	testutil2 "github.com/cosmos/cosmos-sdk/x/staking/client/testutil"
	"testing"
...
func TestCLICreateValidator(t *testing.T) {
	...
	success, stdout, stderr := testutil2.TxStakingCreateValidator(f, barAddr.String(), consPubKey, sdk.NewInt64Coin(cli.Denom, 2), "--generate-only")
	...
	success, _, _ = testutil2.TxStakingCreateValidator(f, barAddr.String(), consPubKey, sdk.NewCoin(cli.Denom, newValTokens), "--dry-run")
	require.True(t, success)

	// Create the validator
	testutil2.TxStakingCreateValidator(f, cli.KeyBar, consPubKey, sdk.NewCoin(cli.Denom, newValTokens), "-y")
	tests.WaitForNextNBlocksTM(1, f.Port)

	// Ensure funds were deducted properly
	require.Equal(t, sendTokens.Sub(newValTokens), testutil.QueryBalances(f, barAddr).AmountOf(cli.Denom))

	// Ensure that validator state is as expected
	validator := testutil2.QueryStakingValidator(f, barVal)
	...
	validatorDelegations := testutil2.QueryStakingDelegationsTo(f, barVal)
	require.Len(t, validatorDelegations, 1)
	require.NotZero(t, validatorDelegations[0].Shares)

	// unbond a single share
	unbondAmt := sdk.NewCoin(sdk.DefaultBondDenom, sdk.TokensFromConsensusPower(1))
	success = testutil2.TxStakingUnbond(f, cli.KeyBar, unbondAmt.String(), barVal, "-y")
	require.True(t, success)
	tests.WaitForNextNBlocksTM(1, f.Port)
	...
	validatorUbds := testutil2.QueryStakingUnbondingDelegationsFrom(f, barVal)
