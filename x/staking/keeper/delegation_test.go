...
func (s *KeeperTestSuite) TestDelegationsByValIndex() {
	...
	_, err = s.msgServer.Delegate(ctx, stakingtypes.NewMsgDelegate(addrDels[1].String(), s.valAddressToString(valAddrs[0]), sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(4))))
	require.NoError(err)
	...
	_, err = s.msgServer.Undelegate(ctx, stakingtypes.NewMsgUndelegate(addrDels[0].String(), s.valAddressToString(valAddrs[0]), sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(1))))
	require.NoError(err)
	...
	_, err = s.msgServer.Undelegate(ctx, stakingtypes.NewMsgUndelegate(addrDels[1].String(), s.valAddressToString(valAddrs[0]), sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(2))))
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUnbondDelegation() {
	...
	delegation := stakingtypes.NewDelegation(delAddrs[0].String(), s.valAddressToString(valAddrs[0]), issuedShares)
...
func (s *KeeperTestSuite) TestUndelegateSelfDelegationBelowMinSelfDelegation() {
	...
	selfDelegation := stakingtypes.NewDelegation(sdk.AccAddress(valAddrs[0].Bytes()).String(), s.valAddressToString(valAddrs[0]), issuedShares)
	...
	validator = stakingkeeper.TestingUpdateValidator(keeper, ctx, validator, true)
	delegation := stakingtypes.NewDelegation(addrDels[0].String(), s.valAddressToString(valAddrs[0]), issuedShares)
	require.NoError(keeper.SetDelegation(ctx, delegation))
...
func (s *KeeperTestSuite) TestUndelegateFromUnbondingValidator() {
	...
	selfDelegation := stakingtypes.NewDelegation(addrDels[0].String(), s.valAddressToString(addrVals[0]), issuedShares)
	require.NoError(keeper.SetDelegation(ctx, selfDelegation))
	...
	delegation := stakingtypes.NewDelegation(addrDels[1].String(), s.valAddressToString(addrVals[0]), issuedShares)
	require.NoError(keeper.SetDelegation(ctx, delegation))
...
func (s *KeeperTestSuite) TestUndelegateFromUnbondedValidator() {
	...
	val0AccAddr := sdk.AccAddress(addrVals[0])
	selfDelegation := stakingtypes.NewDelegation(val0AccAddr.String(), s.valAddressToString(addrVals[0]), issuedShares)
	require.NoError(keeper.SetDelegation(ctx, selfDelegation))
...
func (s *KeeperTestSuite) TestRedelegateSelfDelegation() {
	...
	delegation := stakingtypes.NewDelegation(addrDels[0].String(), s.valAddressToString(addrVals[0]), issuedShares)
	require.NoError(keeper.SetDelegation(ctx, delegation))
...
func (s *KeeperTestSuite) TestRedelegateFromUnbondedValidator() {
	...
	validator = stakingkeeper.TestingUpdateValidator(keeper, ctx, validator, true)
	val0AccAddr := sdk.AccAddress(valAddrs[0].Bytes())
	selfDelegation := stakingtypes.NewDelegation(val0AccAddr.String(), s.valAddressToString(valAddrs[0]), issuedShares)
	require.NoError(keeper.SetDelegation(ctx, selfDelegation))
	...
	require.Equal(delTokens, issuedShares.RoundInt())
	stakingkeeper.TestingUpdateValidator(keeper, ctx, validator, true)
	delegation := stakingtypes.NewDelegation(addrDels[1].String(), s.valAddressToString(valAddrs[0]), issuedShares)
	require.NoError(keeper.SetDelegation(ctx, delegation))
...
func (s *KeeperTestSuite) TestUndelegateWithDustShare() {
	...
	// first add a validators[0] to delegate too
	bond1to1 := stakingtypes.NewDelegation(addrDels[0].String(), s.valAddressToString(valAddrs[0]), math.LegacyNewDec(100))
	require.NoError(keeper.SetDelegation(ctx, bond1to1))
	resBond, err := keeper.Delegations.Get(ctx, collections.Join(addrDels[0], valAddrs[0]))
	require.NoError(err)
	require.Equal(bond1to1, resBond)
	...
	bond2to1 := stakingtypes.NewDelegation(addrDels[1].String(), s.valAddressToString(valAddrs[0]), math.LegacyNewDec(1))
	...
	_, err = s.msgServer.Undelegate(ctx, stakingtypes.NewMsgUndelegate(addrDels[0].String(), s.valAddressToString(valAddrs[0]), sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(99))))
	require.NoError(err)
