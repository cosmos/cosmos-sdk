...
func createValidators(t *testing.T, ctx sdk.Context, app *simapp.SimApp, powers []int64) ([]sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	...
	require.NoError(t, err)
	err = delegateCoinsFromAccount(ctx, app, addrs[0], app.StakingKeeper.TokensFromConsensusPower(ctx, powers[2]), val2)
	require.NoError(t, err)
...
func (suite *KeeperTestSuite) TestGRPCQueryQueuedUndelegation() {
	...
	testCases := []struct {
		...
		{"valid request",
			func() {
				req = &types.QueryQueuedMsgUndelegateRequest{DelegatorAddr: addrAcc.String(), Pagination: &query.PageRequest{Limit: 1, CountTotal: true}}
			},
			true,
		},
	}
