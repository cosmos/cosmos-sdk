...
func (suite *KeeperTestSuite) TestTokensToConsensusPower() {
	suite.Require().Equal(int64(0), suite.app.StakingKeeper.TokensToConsensusPower(suite.ctx, sdk.DefaultPowerReduction.Sub(sdkmath.NewInt(1))))
...
func (suite *KeeperTestSuite) TestTokensFromConsensusPower() {
	suite.Require().Equal(sdkmath.NewInt(0), suite.app.StakingKeeper.TokensFromConsensusPower(suite.ctx, 0))
