...
func TestMsgDecode(t *testing.T) {
	...
	msg, err := types.NewMsgCreateValidator(valAddr1, pk1, coinPos, types.Description{}, commission1, sdk.OneInt(), *randomEVMAddress)
	require.NoError(t, err)
...
func TestMsgCreateValidator(t *testing.T) {
	...
	for _, tc := range tests {
		...
		randomEVMAddress, err := teststaking.RandomEVMAddress()
		require.NoError(t, err)
		msg, err := types.NewMsgCreateValidator(tc.validatorAddr, tc.pubkey, tc.bond, description, tc.CommissionRates, tc.minSelfDelegation, *randomEVMAddress)
		require.NoError(t, err)
		if tc.expectPass {
...
func TestMsgEditValidator(t *testing.T) {
	...
	for _, tc := range tests {
		description := types.NewDescription(tc.moniker, tc.identity, tc.website, tc.securityContact, tc.details)
		newRate := sdk.ZeroDec()
		msg := types.NewMsgEditValidator(tc.validatorAddr, description, &newRate, &tc.minSelfDelegation, nil)
		if tc.expectPass {
			require.Nil(t, msg.ValidateBasic(), "test: %v", tc.name)
		} else {
			require.NotNil(t, msg.ValidateBasic(), "test: %v", tc.name)
