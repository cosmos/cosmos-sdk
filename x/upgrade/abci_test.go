...
func TestRequireName(t *testing.T) {
	...
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{}})
...
func TestRequireFutureTime(t *testing.T) {
	s := setupTest(10, map[int64]bool{})
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "test", Time: s.ctx.BlockHeader().Time}})
	require.NotNil(t, err)
	require.True(t, errors.Is(sdkerrors.ErrInvalidRequest, err), err)
}
...
func TestRequireFutureBlock(t *testing.T) {
	s := setupTest(10, map[int64]bool{})
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "test", Height: s.ctx.BlockHeight()}})
	require.NotNil(t, err)
	require.True(t, errors.Is(sdkerrors.ErrInvalidRequest, err), err)
}
...
func TestCantSetBothTimeAndHeight(t *testing.T) {
	s := setupTest(10, map[int64]bool{})
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "test", Time: time.Now(), Height: s.ctx.BlockHeight() + 1}})
	require.NotNil(t, err)
	require.True(t, errors.Is(sdkerrors.ErrInvalidRequest, err), err)
}
...
func TestDoTimeUpgrade(t *testing.T) {
	...
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "test", Time: time.Now()}})
	require.Nil(t, err)
...
func TestDoHeightUpgrade(t *testing.T) {
	s := setupTest(10, map[int64]bool{})
	t.Log("Verify can schedule an upgrade")
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "test", Height: s.ctx.BlockHeight() + 1}})
	require.Nil(t, err)
...
func TestCanOverwriteScheduleUpgrade(t *testing.T) {
	...
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "bad_test", Height: s.ctx.BlockHeight() + 10}})
	require.Nil(t, err)
	err = s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "test", Height: s.ctx.BlockHeight() + 1}})
	require.Nil(t, err)
...
func VerifyDoUpgrade(t *testing.T) {
	...
	s.keeper.SetUpgradeHandler("test", func(ctx sdk.Context, plan upgradetypes.Plan) {})
...
func VerifyDoUpgradeWithCtx(t *testing.T, newCtx sdk.Context, proposalName string) {
	...
	t.Log("Verify that the upgrade can be successfully applied with a handler")
	s.keeper.SetUpgradeHandler(proposalName, func(ctx sdk.Context, plan upgradetypes.Plan) {})
	require.NotPanics(t, func() {
		s.module.BeginBlock(newCtx, req)
	})
...
func TestHaltIfTooNew(t *testing.T) {
	...
	var called int
	s.keeper.SetUpgradeHandler("future", func(ctx sdk.Context, plan upgradetypes.Plan) { called++ })
	...
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "future", Height: s.ctx.BlockHeight() + 3}})
...
func VerifyCleared(t *testing.T, newCtx sdk.Context) {
	...
	bz, err := s.querier(newCtx, []string{upgradetypes.QueryCurrent}, abci.RequestQuery{})
	require.NoError(t, err)
	require.Nil(t, bz)
}
...
func TestCanClear(t *testing.T) {
	...
	err = s.handler(s.ctx, &upgradetypes.CancelSoftwareUpgradeProposal{Title: "cancel"})
	require.Nil(t, err)
...
func TestPlanStringer(t *testing.T) {
  ...
  Info: `, upgradetypes.Plan{Name: "test", Time: ti}.String())
	require.Equal(t, `Upgrade Plan
  Name: test
  Height: 100
  Info: `, upgradetypes.Plan{Name: "test", Height: 100}.String())
}
...
func TestSkipUpgradeSkippingAll(t *testing.T) {
	...
	req := abci.RequestBeginBlock{Header: newCtx.BlockHeader()}
	err := s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop", Plan: upgradetypes.Plan{Name: "test", Height: skipOne}})
	require.NoError(t, err)
	...
	err = s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop2", Plan: upgradetypes.Plan{Name: "test2", Height: skipTwo}})
	require.NoError(t, err)
...
func TestUpgradeSkippingOnlyTwo(t *testing.T) {
	...
	err = s.handler(s.ctx, &upgradetypes.SoftwareUpgradeProposal{Title: "prop3", Plan: upgradetypes.Plan{Name: "test3", Height: skipThree}})
	require.Nil(t, err)
